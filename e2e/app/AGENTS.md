...
`run.sh` is the stable workspace command used by `make test-e2e-app`. It owns cross-product setup and runs every maintained `app/integration_test/*_test.dart` journey in an independent temporary home inside the real Flutter workspace. Explicit absolute journey paths may be passed for focused verification. Set `SKILLSGO_E2E_HUB_RUNTIME=docker` to exercise the alternate containerized Hub runtime locally.
