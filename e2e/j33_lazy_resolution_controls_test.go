...
func TestJ33LazyResolutionControls(t *testing.T) {
	...
	newTag := execInContainer(t, ctx, container, "sh", "-c", "git --git-dir=/e2e/git/group/subgroup/collection update-ref refs/tags/v1.2.0 $(git --git-dir=/e2e/git/group/subgroup/collection rev-parse main)")
	require.Equal(t, 0, newTag.exitCode, newTag.output)
