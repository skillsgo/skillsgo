...
func TestJ31RepositoryIdentityAndSelection(t *testing.T) {
	...
	rootZIP := execInContainer(t, ctx, container, "sh", "-c", "wget -qO /tmp/root.zip http://127.0.0.1:3000/api/v1/"+collection+"/versions/v1.0.0.zip && unzip -l /tmp/root.zip")
	require.Equal(t, 0, rootZIP.exitCode, rootZIP.output)
	require.Contains(t, rootZIP.output, "SKILL.md")
	aggregate := execInContainer(t, ctx, container, "sh", "-c", "wget -qO /tmp/mixed.zip http://127.0.0.1:3000/api/v1/fixtures.test/group/subgroup/mixed/versions/v1.0.0.zip && unzip -l /tmp/mixed.zip")
