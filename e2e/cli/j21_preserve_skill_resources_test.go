/*
 * [INPUT]: Depends on the disposable E2E environment and a real multi-file public Skill artifact.
...
func TestJ21PreserveSkillResources(t *testing.T) {
	...
	result := execCLI(t, ctx, container, "add", testMismatchedNameID+"@main", "--agent", "codex", "--copy", "--yes", "--confirm-risk", "--allow-critical", "--output", "json")
	require.Equal(t, 0, result.exitCode, result.output)
	var installed addResponse
	require.NoError(t, json.Unmarshal([]byte(result.output), &installed))

	relative := filepath.Join("rules", "async-parallel.md")
	target := filepath.Join(sandboxRoot, "project", ".agents", "skills", testMismatchedName, relative)
