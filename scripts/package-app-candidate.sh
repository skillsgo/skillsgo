#!/usr/bin/env bash
# [INPUT]: Depends on one native Flutter Release bundle, the matching bundled CLI, a workspace or explicit App version, Velopack CLI 1.2.0, optional prior channel packages, release-mode signing/notarization credentials, and unzip-compatible package inspection.
# [OUTPUT]: Produces and verifies one platform-layout-aware Velopack candidate or signed production channel for Windows x64, Linux x64, macOS arm64, or macOS x64, optionally appending a version to an existing feed.
...
if [[ "${target}" == "macos" && "${package_mode}" == "release" ]]; then
  shopt -s nullglob
...
readonly signed="$([[ "${package_mode}" == "release" && "${target}" != "linux" ]] && echo true || echo false)"
