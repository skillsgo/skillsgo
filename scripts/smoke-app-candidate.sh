...
readonly smoke_home="${smoke_root}/home"
mkdir -p "${smoke_home}"
app_pid=""
xvfb_pid=""
...
if [[ "${target}" == "macos" ]]; then
  ...
  if [[ "${CI:-}" == "true" ]]; then
    readonly velopack_ci_packages="/var/tmp/velopack/SkillsGo/packages"
    rm -rf -- "${velopack_ci_packages}"
  fi
  chmod 0755 "${portable}"
  (
    cd "${smoke_root}"
  ...
  setsid env HOME="${smoke_home}" XDG_CACHE_HOME="${smoke_home}/.cache" \
    TMPDIR="${smoke_root}" \
    DISPLAY=":${display_number}" APPIMAGE_EXTRACT_AND_RUN=1 \
