#!/usr/bin/env bash
# [INPUT]: Depends on one structurally verified unsigned Velopack candidate produced by package-app-candidate.sh and native desktop process support.
...
if [[ "${target}" == "macos" ]]; then
  ...
  APPIMAGE_EXTRACT_AND_RUN=1 xvfb-run -a "${portable}" >"${smoke_root}/app.log" 2>&1 &
  app_pid=$!
fi
