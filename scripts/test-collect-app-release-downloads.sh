...
# [OUTPUT]: Verifies unsigned installer naming, signed artifact naming, four-download completeness, checksums, missing-PKG failure, and rejection of mixed macOS signing modes.
...
create_channels() {
  ...
  printf 'arm pkg' >"${assets_root}/osx-arm64/SkillsGo-arm64.pkg"
  printf 'intel pkg' >"${assets_root}/osx-x64/SkillsGo-x64.pkg"
...
test ! -e "${signed_downloads}/SkillsGo-macOS-arm64-unsigned.pkg"
test ! -e "${signed_downloads}/SkillsGo-macOS-x64-unsigned.pkg"
...
rm "${incomplete_assets}/osx-x64/SkillsGo-x64.pkg"
if "${collector}" "${incomplete_assets}" "${incomplete_downloads}" >/dev/null 2>&1; then
  echo "Collector accepted a release without a macOS x64 PKG." >&2
  exit 1
fi
