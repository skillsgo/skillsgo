...
import (
	...
	"github.com/skillsgo/skillsgo/registry/cmd/registry/actions"
	"github.com/skillsgo/skillsgo/registry/internal/shutdown"
	"github.com/skillsgo/skillsgo/registry/pkg/build"
	"github.com/skillsgo/skillsgo/registry/pkg/config"
	registrylog "github.com/skillsgo/skillsgo/registry/pkg/log"
)
...
func main() {
	...
	logLvl, err := registrylog.ParseLevel(conf.LogLevel)
	if err != nil {
		stdlog.Fatalf("Could not parse log level %q: %v", conf.LogLevel, err)
	}

	logger := registrylog.New(conf.CloudRuntime, logLvl, conf.LogFormat)
