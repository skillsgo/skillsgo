/*
 * [INPUT]: Uses the public Registry router with an opt-in Testcontainers PostgreSQL Catalog.
 * [OUTPUT]: Specifies PostgreSQL-backed pagination and empty collection response parity at the HTTP boundary.
 * [POS]: Serves as database-portability coverage for the stable Registry discovery wire contract.
...
// normal Registry suite does not require Docker.
func TestCatalogAPIPostgresProtocol(t *testing.T) {
	...
	router.ServeHTTP(first, httptest.NewRequest(http.MethodGet, "/v1/search?q=capability&limit=2", nil))
	...
	router.ServeHTTP(empty, httptest.NewRequest(http.MethodGet, "/v1/search?q=missing", nil))
