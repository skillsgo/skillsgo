...
func TestEnvOverrides(t *testing.T) {
	...
	expConf := &Config{
		...
		Index:            &Index{},
		SkillCacheDir:    filepath.Join(home, ".skillsgo", "registry", "cache"),
	}
...
func TestRedisSentinelDBConfig(t *testing.T) {
	t.Setenv("SKILLSGO_REGISTRY_SINGLE_FLIGHT_TYPE", "redis-sentinel")
	t.Setenv("SKILLSGO_REGISTRY_REDIS_SENTINEL_ENDPOINTS", "127.0.0.1:26379")
	t.Setenv("SKILLSGO_REGISTRY_REDIS_SENTINEL_MASTER_NAME", "mymaster")
	t.Setenv("SKILLSGO_REGISTRY_REDIS_SENTINEL_DB", "5")
...
func getEnvMap(config *Config) map[string]string {
	envVars := map[string]string{
		"SKILLSGO_REGISTRY_ENVIRONMENT":         config.Environment,
		"SKILLSGO_REGISTRY_SKILL_FETCH_WORKERS": strconv.Itoa(config.SkillFetchWorkers),
		"SKILLSGO_REGISTRY_PROTOCOL_WORKERS":    strconv.Itoa(config.ProtocolWorkers),
		"SKILLSGO_REGISTRY_LOG_LEVEL":           config.LogLevel,
		"SKILLSGO_REGISTRY_CLOUD_RUNTIME":       config.CloudRuntime,
		"SKILLSGO_REGISTRY_TIMEOUT":             strconv.Itoa(config.Timeout),
	}

	envVars["SKILLSGO_REGISTRY_STORAGE_TYPE"] = config.StorageType
	envVars["SKILLSGO_REGISTRY_GLOBAL_ENDPOINT"] = config.GlobalEndpoint
	envVars["SKILLSGO_REGISTRY_PORT"] = config.Port
	envVars["SKILLSGO_REGISTRY_ENABLE_PPROF"] = strconv.FormatBool(config.EnablePprof)
	envVars["SKILLSGO_REGISTRY_PPROF_PORT"] = config.PprofPort
	envVars["SKILLSGO_REGISTRY_BASIC_AUTH_USER"] = config.BasicAuthUser
	envVars["SKILLSGO_REGISTRY_BASIC_AUTH_PASS"] = config.BasicAuthPass
	envVars["SKILLSGO_REGISTRY_FORCE_SSL"] = strconv.FormatBool(config.ForceSSL)
	envVars["SKILLSGO_REGISTRY_HOME_TEMPLATE_PATH"] = config.HomeTemplatePath
	envVars["SKILLSGO_REGISTRY_PROXY_VALIDATOR"] = config.ValidatorHook
	envVars["SKILLSGO_REGISTRY_PATH_PREFIX"] = config.PathPrefix
	envVars["SKILLSGO_REGISTRY_NETRC_PATH"] = config.NETRCPath
	envVars["SKILLSGO_REGISTRY_HGRC_PATH"] = config.HGRCPath
	envVars["SKILLSGO_REGISTRY_ROBOTS_FILE"] = config.RobotsFile

	storage := config.Storage
	if storage != nil {
		if storage.Disk != nil {
			envVars["SKILLSGO_REGISTRY_DISK_STORAGE_ROOT"] = storage.Disk.RootPath
		}
		if storage.GCP != nil {
			envVars["GOOGLE_CLOUD_PROJECT"] = storage.GCP.ProjectID
			envVars["SKILLSGO_REGISTRY_STORAGE_GCP_BUCKET"] = storage.GCP.Bucket
		}
		if storage.Minio != nil {
			envVars["SKILLSGO_REGISTRY_MINIO_ENDPOINT"] = storage.Minio.Endpoint
			envVars["SKILLSGO_REGISTRY_MINIO_ACCESS_KEY_ID"] = storage.Minio.Key
			envVars["SKILLSGO_REGISTRY_MINIO_SECRET_ACCESS_KEY"] = storage.Minio.Secret
			envVars["SKILLSGO_REGISTRY_MINIO_USE_SSL"] = strconv.FormatBool(storage.Minio.EnableSSL)
			envVars["SKILLSGO_REGISTRY_MINIO_REGION"] = storage.Minio.Region
			envVars["SKILLSGO_REGISTRY_MINIO_BUCKET_NAME"] = storage.Minio.Bucket
		}
		if storage.Mongo != nil {
			envVars["SKILLSGO_REGISTRY_MONGO_STORAGE_URL"] = storage.Mongo.URL
			envVars["SKILLSGO_REGISTRY_MONGO_CERT_PATH"] = storage.Mongo.CertPath
			envVars["SKILLSGO_REGISTRY_MONGO_INSECURE"] = strconv.FormatBool(storage.Mongo.InsecureConn)
			envVars["SKILLSGO_REGISTRY_MONGO_DEFAULT_DATABASE"] = storage.Mongo.DefaultDBName
			envVars["SKILLSGO_REGISTRY_MONGO_DEFAULT_COLLECTION"] = storage.Mongo.DefaultCollectionName
		...
		if storage.S3 != nil {
			...
			envVars["SKILLSGO_REGISTRY_S3_BUCKET_NAME"] = storage.S3.Bucket
		}
	}
	...
	if singleFlight != nil {
		if singleFlight.Redis != nil {
			envVars["SKILLSGO_REGISTRY_SINGLE_FLIGHT_TYPE"] = "redis"
			envVars["SKILLSGO_REGISTRY_REDIS_ENDPOINT"] = singleFlight.Redis.Endpoint
			envVars["SKILLSGO_REGISTRY_REDIS_PASSWORD"] = singleFlight.Redis.Password
			if singleFlight.Redis.LockConfig != nil {
				envVars["SKILLSGO_REGISTRY_REDIS_LOCK_TTL"] = strconv.Itoa(singleFlight.Redis.LockConfig.TTL)
				envVars["SKILLSGO_REGISTRY_REDIS_LOCK_TIMEOUT"] = strconv.Itoa(singleFlight.Redis.LockConfig.Timeout)
				envVars["SKILLSGO_REGISTRY_REDIS_LOCK_MAX_RETRIES"] = strconv.Itoa(singleFlight.Redis.LockConfig.MaxRetries)
			}
		} else if singleFlight.RedisSentinel != nil {
			envVars["SKILLSGO_REGISTRY_SINGLE_FLIGHT_TYPE"] = "redis-sentinel"
			envVars["SKILLSGO_REGISTRY_REDIS_SENTINEL_ENDPOINTS"] = strings.Join(singleFlight.RedisSentinel.Endpoints, ",")
			envVars["SKILLSGO_REGISTRY_REDIS_SENTINEL_MASTER_NAME"] = singleFlight.RedisSentinel.MasterName
			envVars["SKILLSGO_REGISTRY_REDIS_SENTINEL_PASSWORD"] = singleFlight.RedisSentinel.SentinelPassword
			envVars["SKILLSGO_REGISTRY_REDIS_USERNAME"] = singleFlight.RedisSentinel.RedisUsername
			envVars["SKILLSGO_REGISTRY_REDIS_PASSWORD"] = singleFlight.RedisSentinel.RedisPassword
			envVars["SKILLSGO_REGISTRY_REDIS_SENTINEL_DB"] = strconv.Itoa(singleFlight.RedisSentinel.DB)
			if singleFlight.RedisSentinel.LockConfig != nil {
				envVars["SKILLSGO_REGISTRY_REDIS_LOCK_TTL"] = strconv.Itoa(singleFlight.RedisSentinel.LockConfig.TTL)
				envVars["SKILLSGO_REGISTRY_REDIS_LOCK_TIMEOUT"] = strconv.Itoa(singleFlight.RedisSentinel.LockConfig.Timeout)
				envVars["SKILLSGO_REGISTRY_REDIS_LOCK_MAX_RETRIES"] = strconv.Itoa(singleFlight.RedisSentinel.LockConfig.MaxRetries)
			}
		} else if singleFlight.Etcd != nil {
			envVars["SKILLSGO_REGISTRY_SINGLE_FLIGHT_TYPE"] = "etcd"
			envVars["SKILLSGO_REGISTRY_ETCD_ENDPOINTS"] = singleFlight.Etcd.Endpoints
		} else if singleFlight.GCP != nil {
			envVars["SKILLSGO_REGISTRY_GCP_STALE_THRESHOLD"] = strconv.Itoa(singleFlight.GCP.StaleThreshold)
		}
	}
	return envVars
}
...
func testDecode(t *testing.T, tc decodeTestCase) {
	t.Setenv("SKILLSGO_REGISTRY_LIST_TEST", tc.given)

	var config struct {
		GoBinaryEnvVars EnvList `envconfig:"SKILLSGO_REGISTRY_LIST_TEST"`
	}
