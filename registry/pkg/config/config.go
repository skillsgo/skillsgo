...
type Config struct {
	...
	Environment           string    `envconfig:"SKILLSGO_REGISTRY_ENVIRONMENT" validate:"required"`
	SkillFetchWorkers     int       `envconfig:"SKILLSGO_REGISTRY_SKILL_FETCH_WORKERS"           validate:"required"`
	SkillCacheDir         string    `ignored:"true"`
	ProtocolWorkers       int       `envconfig:"SKILLSGO_REGISTRY_PROTOCOL_WORKERS"        validate:"required"`
	LogLevel              string    `envconfig:"SKILLSGO_REGISTRY_LOG_LEVEL"               validate:"required"`
	LogFormat             string    `envconfig:"SKILLSGO_REGISTRY_LOG_FORMAT"              validate:"oneof='' 'json' 'plain'"`
	CloudRuntime          string    `envconfig:"SKILLSGO_REGISTRY_CLOUD_RUNTIME"           validate:"required_without=LogFormat"`
	EnablePprof           bool      `envconfig:"SKILLSGO_REGISTRY_ENABLE_PPROF"`
	PprofPort             string    `envconfig:"SKILLSGO_REGISTRY_PPROF_PORT"`
	FilterFile            string    `envconfig:"SKILLSGO_REGISTRY_FILTER_FILE"`
	TraceExporterURL      string    `envconfig:"SKILLSGO_REGISTRY_TRACE_EXPORTER_URL"`
	TraceExporter         string    `envconfig:"SKILLSGO_REGISTRY_TRACE_EXPORTER"`
	TraceSamplingFraction float64   `envconfig:"SKILLSGO_REGISTRY_TRACE_SAMPLING_FRACTION"`
	StatsExporter         string    `envconfig:"SKILLSGO_REGISTRY_STATS_EXPORTER"`
	StorageType           string    `envconfig:"SKILLSGO_REGISTRY_STORAGE_TYPE"            validate:"required"`
	GlobalEndpoint        string    `envconfig:"SKILLSGO_REGISTRY_GLOBAL_ENDPOINT"` // This feature is not yet implemented
	Port                  string    `envconfig:"SKILLSGO_REGISTRY_PORT"`
	UnixSocket            string    `envconfig:"SKILLSGO_REGISTRY_UNIX_SOCKET"`
	BasicAuthUser         string    `envconfig:"SKILLSGO_REGISTRY_BASIC_AUTH_USER"`
	BasicAuthPass         string    `envconfig:"SKILLSGO_REGISTRY_BASIC_AUTH_PASS"`
	HomeTemplatePath      string    `envconfig:"SKILLSGO_REGISTRY_HOME_TEMPLATE_PATH"`
	ForceSSL              bool      `envconfig:"SKILLSGO_REGISTRY_FORCE_SSL"`
	ValidatorHook         string    `envconfig:"SKILLSGO_REGISTRY_PROXY_VALIDATOR"`
	PathPrefix            string    `envconfig:"SKILLSGO_REGISTRY_PATH_PREFIX"`
	NETRCPath             string    `envconfig:"SKILLSGO_REGISTRY_NETRC_PATH"`
	GithubToken           string    `envconfig:"SKILLSGO_REGISTRY_GITHUB_TOKEN"`
	HGRCPath              string    `envconfig:"SKILLSGO_REGISTRY_HGRC_PATH"`
	TLSCertFile           string    `envconfig:"SKILLSGO_REGISTRY_TLSCERT_FILE"`
	TLSKeyFile            string    `envconfig:"SKILLSGO_REGISTRY_TLSKEY_FILE"`
	DownloadMode          mode.Mode `envconfig:"SKILLSGO_REGISTRY_DOWNLOAD_MODE"`
	DownloadURL           string    `envconfig:"SKILLSGO_REGISTRY_DOWNLOAD_URL"`
	NetworkMode           string    `envconfig:"SKILLSGO_REGISTRY_NETWORK_MODE"            validate:"oneof=strict offline fallback"`
	SingleFlightType      string    `envconfig:"SKILLSGO_REGISTRY_SINGLE_FLIGHT_TYPE"`
	RobotsFile            string    `envconfig:"SKILLSGO_REGISTRY_ROBOTS_FILE"`
	IndexType             string    `envconfig:"SKILLSGO_REGISTRY_INDEX_TYPE"`
	ShutdownTimeout       int       `envconfig:"SKILLSGO_REGISTRY_SHUTDOWN_TIMEOUT"        validate:"min=0"`
	StashTimeout          int       `envconfig:"SKILLSGO_REGISTRY_STASH_TIMEOUT"`
...
//
//	SKILLSGO_REGISTRY_GO_BINARY_ENV_VARS="GOPRIVATE=*.corp.example.com,rsc.io/private; GOPROXY=direct"
//
// See the below link for more information:
// https://github.com/skillsgo/skillsgo/registry/issues/1404
...
func envOverride(config *Config) error {
	...
	config.SkillCacheDir, err = resolveRegistryCacheDir(config.SkillCacheDir)
	if err != nil {
		return err
	}
	...
	config.Database.DSN, err = resolveRegistryDatabaseDSN(config.Database.Type, config.Database.DSN)
	if err != nil {
		return err
	}
	if config.StorageType == "disk" {
		...
		config.Storage.Disk.RootPath, err = resolveRegistryArtifactDir(config.Storage.Disk.RootPath)
		if err != nil {
			return err
		}
	}
