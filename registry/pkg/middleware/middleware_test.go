...
// Avoid import cycle.
const (
	pathList        = "/{skill:.+}/@v/list"
	pathVersionInfo = "/{skill:.+}/@v/{version}.info"
)
...
func newTestFilter(filterFile string) (*skill.Filter, error) {
	...
	f.AddRule("github.com/skillsgo/skillsgo/registry/", nil, skill.Direct)
...
func Test_FilterMiddleware(t *testing.T) {
	...
	res := w.JSON("/github.com/skillsgo/skillsgo/registry/@v/list").Get()
	r.Equal(http.StatusSeeOther, res.Code)
	r.Equal(conf.GlobalEndpoint+"/github.com/skillsgo/skillsgo/registry/@v/list", res.HeaderMap.Get("Location"))
...
func (suite *HookTestsSuite) TestHookOnList() {
	...
	suite.w.JSON("/github.com/skillsgo/skillsgo/registry/@v/list").Get()
