...
import (
	...
	"github.com/skillsgo/skillsgo/registry/pkg/storage/compliance"
...
func getTestConfig(containerName string) *config.AzureBlobConfig {
	key := os.Getenv("SKILLSGO_REGISTRY_AZURE_ACCOUNT_KEY")
	resourceId := os.Getenv("SKILLSGO_REGISTRY_AZURE_MANAGED_IDENTITY_RESOURCE_ID")
	credentialScope := os.Getenv("SKILLSGO_REGISTRY_AZURE_CREDENTIAL_SCOPE")
	if key == "" && (resourceId == "" || credentialScope == "") {
		return nil
	}
	name := os.Getenv("SKILLSGO_REGISTRY_AZURE_ACCOUNT_NAME")
	if name == "" {
		return nil
	}
