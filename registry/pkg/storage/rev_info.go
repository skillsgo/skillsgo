...
type Origin struct {
	...
	// If Ref is non-empty, then the resolution of this Skill version
	// depends on Ref resolving to the revision identified by CommitSHA.
	// If Ref still resolves to CommitSHA, the Origin is still valid.
	// For Git, the Ref is a full ref like "refs/heads/main" or "refs/tags/v1.2.3",
	// and CommitSHA is the Git commit the ref maps to. TreeSHA identifies the
	// complete contents of the Skill directory within that commit.
	// Other VCS might choose differently, but the idea is that Ref is the name
	// with a mutable meaning while the SHAs have immutable meanings.
