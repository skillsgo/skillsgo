...
2. Provide the token to the Athens proxy either through the [config.toml](https://github.com/gomods/athens/blob/main/config.dev.toml) file (the `GithubToken` field) or by setting the `SKILLSGO_REGISTRY_GITHUB_TOKEN` environment variable.
