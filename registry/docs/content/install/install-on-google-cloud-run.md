...
Change the values of these environment variables to be appropriate for your application. For `GOOGLE_CLOUD_PROJECT`, this needs to be the name of the project that has your cloud run deployment in it. `SKILLSGO_REGISTRY_REGION` should be the [region](https://cloud.google.com/about/locations/?tab=americas) that your cloud run instance will be in, and `GCS_BUCKET` should be the Google Cloud Storage bucket that Athens will store module code and metadata in..

```console
$ export GOOGLE_CLOUD_PROJECT=your-project
$ export SKILLSGO_REGISTRY_REGION=asia-northeast1
...
$ gcloud beta run deploy \
    ...
    --platform managed \
    --region $SKILLSGO_REGISTRY_REGION \
    --allow-unauthenticated \
    --set-env-vars=SKILLSGO_REGISTRY_STORAGE_TYPE=gcp \
    --set-env-vars=GOOGLE_CLOUD_PROJECT=$GOOGLE_CLOUD_PROJECT \
    --set-env-vars=SKILLSGO_REGISTRY_STORAGE_GCP_BUCKET=$GCS_BUCKET \
    athens
```
...
```console
$ gcloud beta run services describe athens --platform managed --region $SKILLSGO_REGISTRY_REGION | grep hostname
```
