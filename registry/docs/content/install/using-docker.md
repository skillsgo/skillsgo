...
Then we need to set the `SKILLSGO_REGISTRY_STORAGE_TYPE` and `SKILLSGO_REGISTRY_DISK_STORAGE_ROOT` environment variables when we run the Docker container.

**Bash**
```bash
export SKILLSGO_REGISTRY_STORAGE=$HOME/athens-storage
...
**PowerShell**
```PowerShell
$env:SKILLSGO_REGISTRY_STORAGE = "$(Join-Path $HOME athens-storage)"
