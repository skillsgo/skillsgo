...
3.  Once we've properly authenticated we want to share the .subversion directory with the Athens proxy server in order to reuse those credentials. Below we're setting it as a volume on our proxy container.
    ...
    **Bash**
    ...
    ```bash
    export SKILLSGO_REGISTRY_STORAGE=~/athens-storage
    export SKILLSGO_REGISTRY_SVN=~/.subversion
    mkdir -p $SKILLSGO_REGISTRY_STORAGE
    docker run -d -v $SKILLSGO_REGISTRY_STORAGE:/var/lib/athens \
      -v $SKILLSGO_REGISTRY_SVN:/root/.subversion \
      -e SKILLSGO_REGISTRY_DISK_STORAGE_ROOT=/var/lib/athens \
      -e SKILLSGO_REGISTRY_STORAGE_TYPE=disk \
      --name athens-proxy \
      --restart always \
      -p 3000:3000 \
    ...
    **PowerShell**
    ...
    ```PowerShell
    $env:SKILLSGO_REGISTRY_STORAGE = "$(Join-Path $pwd athens-storage)"
    $env:SKILLSGO_REGISTRY_SVN = "$(Join-Path $pwd .subversion)"
    md -Path $env:SKILLSGO_REGISTRY_STORAGE
    docker run -d -v "$($env:SKILLSGO_REGISTRY_STORAGE):/var/lib/athens" `
      -v "$($env:SKILLSGO_REGISTRY_SVN):/root/.subversion" `
      -e SKILLSGO_REGISTRY_DISK_STORAGE_ROOT=/var/lib/athens `
      -e SKILLSGO_REGISTRY_STORAGE_TYPE=disk `
      --name athens-proxy `
      --restart always `
...
5. Once we've properly setup our authentication we want to share the bazaar configuration directory with the Athens proxy server in order to reuse those credentials. Below we're setting it as a volume on our proxy container.
   ...
   **Bash**
   ...
   ```bash
   export SKILLSGO_REGISTRY_STORAGE=~/athens-storage
   export SKILLSGO_REGISTRY_BZR=~/.bazaar
   mkdir -p $SKILLSGO_REGISTRY_STORAGE
   docker run -d -v $SKILLSGO_REGISTRY_STORAGE:/var/lib/athens \
     -v $SKILLSGO_REGISTRY_BZR:/root/.bazaar \
     -e SKILLSGO_REGISTRY_DISK_STORAGE_ROOT=/var/lib/athens \
     -e SKILLSGO_REGISTRY_STORAGE_TYPE=disk \
     --name athens-proxy \
     --restart always \
   ...
   **PowerShell**
   ...
   ```PowerShell
   $env:SKILLSGO_REGISTRY_STORAGE = "$(Join-Path $pwd athens-storage)"
   $env:SKILLSGO_REGISTRY_BZR = "$(Join-Path $pwd .bazaar)"
   md -Path $env:SKILLSGO_REGISTRY_STORAGE
   docker run -d -v "$($env:SKILLSGO_REGISTRY_STORAGE):/var/lib/athens" `
     -v "$($env:SKILLSGO_REGISTRY_BZR):/root/.bazaar" `
     -e SKILLSGO_REGISTRY_DISK_STORAGE_ROOT=/var/lib/athens `
     -e SKILLSGO_REGISTRY_STORAGE_TYPE=disk `
     --name athens-proxy `
     --restart always `
...
```bash
$ mkdir -p storage
$ SKILLSGO_REGISTRY_STORAGE=storage
$ docker run --rm -d \
    -v "$PWD/$SKILLSGO_REGISTRY_STORAGE:/var/lib/athens" \
    -v "$PWD/gitconfig/.gitconfig:/root/.gitconfig" \
    -v "$PWD/ssh-keys:/root/.ssh" \
    -e SKILLSGO_REGISTRY_DISK_STORAGE_ROOT=/var/lib/athens -e SKILLSGO_REGISTRY_STORAGE_TYPE=disk --name athens-proxy -p 3000:3000 gomods/athens:canary
```
