...
All of them can be configured using `config.toml` file. You need to set a valid driver in `StorageType` value or you can set it in environment variable `SKILLSGO_REGISTRY_STORAGE_TYPE` on your server.
...
##### Configuration:
    ...
    # Env override: SKILLSGO_REGISTRY_STORAGE_TYPE
    StorageType = "memory"
...
where `/path/on/disk` is your desired location. Also it can be set using `SKILLSGO_REGISTRY_DISK_STORAGE_ROOT` env
...
##### Configuration:
    ...
    [Storage]
        [Storage.Mongo]
            ...
            # Env override: SKILLSGO_REGISTRY_MONGO_STORAGE_URL
            URL = "mongodb://127.0.0.1:27017"

            # Not required parameter
            # Path to certificate to use for the mongo connection
            # Env override: SKILLSGO_REGISTRY_MONGO_CERT_PATH
            ...
            # Env override: SKILLSGO_REGISTRY_MONGO_INSECURE
            Insecure = false

            # Not required parameter
            # Allows for use of custom database 
            # Env override: SKILLSGO_REGISTRY_MONGO_DEFAULT_DATABASE
            DefaultDBName = athens

            # Not required parameter
            # Allows for use of custom collection 
            # Env override: SKILLSGO_REGISTRY_MONGO_DEFAULT_COLLECTION
...
##### Configuration:
    ...
    [Storage]
        [Storage.GCP]
            ...
            # Env override: SKILLSGO_REGISTRY_STORAGE_GCP_BUCKET
...
##### Configuration:
    ...
    [Storage]
            ...
            # Env override: SKILLSGO_REGISTRY_S3_BUCKET_NAME
...
##### Configuration:
    ...
    [Storage]
        [Storage.Minio]
            ...
            # Env override: SKILLSGO_REGISTRY_MINIO_ENDPOINT
            Endpoint = "127.0.0.1:9001"

            # Access Key for Minio storage
            # Env override: SKILLSGO_REGISTRY_MINIO_ACCESS_KEY_ID
            Key = "YOUR_MINIO_SECRET_KEY"

            # Secret Key for Minio storage
            # Env override: SKILLSGO_REGISTRY_MINIO_SECRET_ACCESS_KEY
            Secret = "YOUR_MINIO_SECRET_KEY"

            # Enable SSL for Minio connections
            # Defaults to true
            # Env override: SKILLSGO_REGISTRY_MINIO_USE_SSL
            EnableSSL = false

            # Minio Bucket to use for storage
            # Env override: SKILLSGO_REGISTRY_MINIO_BUCKET_NAME
            Bucket = "gomods"
...
##### Configuration:
    ...
    [Storage]
        [Storage.Minio]
            ...
            # Env override: SKILLSGO_REGISTRY_MINIO_REGION
...
##### Configuration:
    ...
    [Storage]
        [Storage.External]
            # Env override: SKILLSGO_REGISTRY_EXTERNAL_STORAGE_URL
...
Setting the `SingleFlightType` (or `SKILLSGO_REGISTRY_SINGLE_FLIGHT TYPE` in the environment) configuration
...
The recommend configuration is 3 endpoints, however, more can be used.
    ...
    [SingleFlight]
        [SingleFlight.Etcd]
            # Env override: SKILLSGO_REGISTRY_ETCD_ENDPOINTS
...
You can also optionally specify a password to connect to the redis server with
    ...
    [SingleFlight]
        [SingleFlight.Redis]
            ...
            # Env override: SKILLSGO_REGISTRY_REDIS_ENDPOINT
            Endpoint = "127.0.0.1:6379"

            # Password is the password for the redis instance
            # Env override: SKILLSGO_REGISTRY_REDIS_PASSWORD
            Password = ""
...
If you would like to customize the distributed lock options then you can optionally override the default lock config to better suit your use-case:
    ...
    [SingleFlight.Redis]
        ...
        [SingleFlight.Redis.LockConfig]
            ...
            # Env override: SKILLSGO_REGISTRY_REDIS_LOCK_TTL
            TTL = 900
            # Timeout for acquiring the lock in seconds. Defaults to 15 seconds.
            # Env override: SKILLSGO_REGISTRY_REDIS_LOCK_TIMEOUT
            Timeout = 15
            # Max retries while acquiring the lock. Defaults to 10.
            # Env override: SKILLSGO_REGISTRY_REDIS_LOCK_MAX_RETRIES
            MaxRetries = 10
...
Optionally, like `redis`, you can also specify a password to connect to the `redis-sentinel` endpoints with
    ...
    [SingleFlight]
      [SingleFlight.RedisSentinel]
          ...
          # Env override: SKILLSGO_REGISTRY_REDIS_SENTINEL_ENDPOINTS
          ...
          # Env override: SKILLSGO_REGISTRY_REDIS_SENTINEL_DB
          DB = 0

          # RedisUsername is the username for authenticating with the redis master
          # Env override: SKILLSGO_REGISTRY_REDIS_USERNAME
          RedisUsername = ""
