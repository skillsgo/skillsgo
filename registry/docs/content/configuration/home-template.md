...
Do so by configuring `HomeTemplatePath` via the config or `SKILLSGO_REGISTRY_HOME_TEMPLATE_PATH` to a location on disk with a Go HTML template or placing a template file at `/var/lib/athens/home.html`.
