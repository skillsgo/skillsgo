...
- Set `SKILLSGO_REGISTRY_TRACE_EXPORTER=otlp` and `SKILLSGO_REGISTRY_TRACE_EXPORTER_URL` to your collector's OTLP endpoint (for example `http://localhost:4317`). The `docker-compose.yml` in the source root already wires this up for the `dev` service.
