...
function stopProcesses () {
  Get-Process -Name registry* -ErrorAction SilentlyContinue | Stop-Process -Force
}
...
try {
  ...
  Push-Location $(Join-Path $repoDir cmd | Join-Path -ChildPath registry)
  ## just in case something is still running
  stopProcesses
  & go build -mod=vendor
  Start-Process -NoNewWindow .\registry.exe
