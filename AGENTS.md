...
- The App routes every Hub and local operation through one long-lived bundled CLI Server session. In Cloud mode it may call the independently deployed SkillsGo Cloud origin declared by `skillsgo hub info` for Cloud-owned ranking reads; it never calls Hub HTTP directly.
