...
- `runtime.go`: defines type-safe River job/finalizer registration, bounded source/default/maintenance worker allocation, exact active-job lookup for domain reconciliation, sleep-first periodic submission, burst startup/idle shutdown, future-job wake scheduling, configurable failure detection, and PostgreSQL execution without a generic envelope.
