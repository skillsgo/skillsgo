 ...
 * [OUTPUT]: Provides the mongo package behavior implemented by mongo.go.
...
// SkillStore represents a mongo backed storage backend.
...
func NewStorage(conf *config.MongoConfig, timeout time.Duration) (*SkillStore, error) {
	const op errors.Op = "mongo.NewStorage"
	if conf == nil {
		return nil, errors.E(op, "No Mongo Configuration provided")
	}
	ms := &SkillStore{url: conf.URL, certPath: conf.CertPath, timeout: timeout, insecure: conf.InsecureConn}
...
func (s *SkillStore) initDatabase() *mongo.Collection {
	if s.db == "" {
		s.db = "athens"
	}
...
func (s *SkillStore) newClient() (*mongo.Client, error) {
...
func (s *SkillStore) gridFileName(mod, ver string) string {
