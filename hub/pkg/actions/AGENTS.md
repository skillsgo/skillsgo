...
- `package_publisher.go`: validates complete Package Artifacts, computes description/document digests and source language once at publication, prepares typed immutable Package Info plus content-addressed Skill source objects, coordinates bounded and negative-cached Source Repository work, and batches Historical Publications by consuming one explicitly leased Skill-source Backfill session plus one Artifact session while retaining ordinary single-Version publication.
