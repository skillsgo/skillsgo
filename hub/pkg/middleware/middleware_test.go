...
func Test_FilterMiddleware(t *testing.T) {
	...
	res := request("/github.com/skillsgo/skillsgo/hub/versions")
	r.Equal(http.StatusBadGateway, res.StatusCode)

	// Excluded, expects a 403
	res = request("/github.com/athens-artifacts/no-tags/versions")
	r.Equal(http.StatusForbidden, res.StatusCode)

	// Private, the proxy is working and returns a 200
	res = request("/github.com/athens-artifacts/happy-path/versions")
...
func (suite *HookTestsSuite) TestHookOnList() {
	...
	_, _ = suite.w.Test(mustRequest(suite.T(), "/github.com/skillsgo/skillsgo/hub/versions"))
...
func (suite *HookTestsSuite) TestHookPass() {
	...
	res, _ := suite.w.Test(mustRequest(suite.T(), "/github.com/athens-artifacts/happy-path/versions/v1.0.0"))
