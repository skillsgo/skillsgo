 ...
 * [OUTPUT]: Specifies the download package behavior covered by list_merge_test.go.
...
import (
	...
	athenserr "github.com/skillsgo/skillsgo/hub/pkg/errors"
...
var listMergeTests = []listMergeTest{
	...
	},
	{
		...
		strErr:      nil,
		expected:    nil,
		expectedErr: athenserr.E(testOp, athenserr.S(testModName), athenserr.KindNotFound, errors.New("remote: Repository not found")),
	},
	{
	...
	for _, tc := range listMergeTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, v := range tc.strVersions {
				s.Save(ctx, testModName, v, io.NopCloser(bytes.NewReader(bts)), nil, bts)
			}
			defer clearStorage(s, testModName, tc.strVersions)
			dp := New(&Opts{Storage: s, Lister: &listerMock{versions: tc.goVersions, err: tc.goErr}, NetworkMode: Strict})
