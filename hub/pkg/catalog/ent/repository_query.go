...
func (_q *RepositoryQuery) QuerySkills() *SkillQuery {
...
func (_q *RepositoryQuery) WithSkills(opts ...func(*SkillQuery)) *RepositoryQuery {
	query := (&SkillClient{config: _q.config}).Query()
...
//		GroupBy(repository.FieldSourceHost).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Repository.Query().
//		Select(repository.FieldSourceHost).
//		Scan(ctx, &v)
...
func (_q *RepositoryQuery) loadSkills(ctx context.Context, query *SkillQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *Skill)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(skill.FieldRepositoryID)
	}
	query.Where(predicate.Skill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(repository.SkillsColumn), fks...))
	}))
