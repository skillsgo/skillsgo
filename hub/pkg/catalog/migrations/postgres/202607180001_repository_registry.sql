...
-- [POS]: Serves as the PostgreSQL migration to repository-owned public identity and unambiguous telemetry table scope.
...
ALTER TABLE skills ADD COLUMN repository_id BIGINT REFERENCES repositories(id);
UPDATE skills SET repository_id = repositories.id
FROM repositories
WHERE repositories.repository_id = lower(skills.source_host || '/' || skills.repository);
ALTER TABLE skills ALTER COLUMN repository_id SET NOT NULL;
