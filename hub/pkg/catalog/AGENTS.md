...
- `catalog.go`: exposes the Catalog API, reusable Package Version aggregate validation, transactionally priority-gated current Package selection, ordered current-Package update projections, the shared schema-fixed zero-minimum PostgreSQL pgx pool with bounded idle retention and public extension fallback, Package-scoped source-metadata cache state, content-addressed global localization identity, name-first language-consistent Find, exact-path ordered batch hydration, immutable Versions, and ordered Skill membership with persisted source-language provenance.
