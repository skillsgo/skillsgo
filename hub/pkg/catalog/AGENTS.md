...
- `catalog.go`: exposes the Catalog API, reusable Package Version aggregate validation, transactionally priority-gated current Package selection, ordered current-Package update projections, the shared schema-fixed zero-minimum PostgreSQL pgx pool with bounded idle retention and public extension fallback, content-addressed global localization success/failure identity, bounded missing-or-stale translation candidates, name-first language-consistent Find, exact-path ordered batch hydration, immutable Versions, and ordered Skill membership with persisted source-language provenance.
...
- `queries/`: contains the maintained sqlc query source, including set-based current-Package update hydration; SQL used by Catalog business operations belongs here except connection-scoped PostgreSQL advisory locks.
