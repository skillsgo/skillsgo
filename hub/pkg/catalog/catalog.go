/*
 * [INPUT]: Depends on Ent entities, SQLx for dialect-specific discovery queries, versioned Atlas SQL migrations, Hub database configuration, and canonical Skill coordinates.
...
type SkillVersion struct {
	ID            int64     `db:"id" json:"id"`
	SkillID       int64     `db:"skill_id" json:"skillId"`
...
func (c *Catalog) RecordInstall(ctx context.Context, event InstallEvent) (bool, error) {
	...
	var skillID int64
	if err := tx.GetContext(ctx, &skillID, c.db.Rebind("SELECT id FROM skills WHERE coordinate = ?"), event.Coordinate); err != nil {
		return false, err
	}
...
func (c *Catalog) UpsertSkill(ctx context.Context, skill *Skill) error {
	...
	stored, err := c.orm.Skill.Create().
		SetCoordinate(skill.Coordinate).SetName(skill.Name).SetDescription(skill.Description).
		...
		OnConflictColumns(entskill.FieldCoordinate).UpdateNewValues().ID(ctx)
	if err == nil {
		skill.ID = stored
	}
	return err
...
func (c *Catalog) UpdateGitHubStars(ctx context.Context, coordinate string, stars int64) error {
	if stars < 0 {
		return fmt.Errorf("github stars cannot be negative")
	}
	_, err := c.orm.Skill.Update().Where(entskill.CoordinateEQ(coordinate)).SetGithubStars(stars).Save(ctx)
	return err
}

func (c *Catalog) TotalInstalls(ctx context.Context, skillID int64) (int64, error) {
	var total int64
...
func (c *Catalog) Skill(ctx context.Context, coordinate string) (*Skill, error) {
	stored, err := c.orm.Skill.Query().Where(entskill.CoordinateEQ(coordinate)).Only(ctx)
	if err != nil {
...
func (c *Catalog) RecordSkillVersion(ctx context.Context, coordinate string, candidate SkillVersion) (*SkillVersion, error) {
	...
	storedSkill, err := c.orm.Skill.Query().Where(entskill.CoordinateEQ(coordinate)).Only(ctx)
	if err != nil {
		return nil, err
	}
	...
	_, err = c.orm.SkillVersion.Create().SetSkillID(skillID).SetVersion(candidate.Version).
	...
	entity, err := c.orm.SkillVersion.Query().Where(entskillversion.And(
		entskillversion.SkillIDEQ(skillID), entskillversion.VersionEQ(candidate.Version),
	)).Only(ctx)
	if err != nil {
		return nil, err
	}
...
func skillFromEnt(entity *catalogent.Skill) *Skill {
	return &Skill{ID: entity.ID, Coordinate: entity.Coordinate, Name: entity.Name, Description: entity.Description,
...
func skillVersionFromEnt(entity *catalogent.SkillVersion) *SkillVersion {
	return &SkillVersion{ID: entity.ID, SkillID: entity.SkillID, Version: entity.Version, CommitSHA: entity.CommitSha,
