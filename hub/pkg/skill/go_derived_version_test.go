...
func TestGoDerivedTagAndRevisionAmbiguityMatrix(t *testing.T) {
	tests := []struct {
		...
		{name: "non-SemVer Tag is ignored by main", prepare: func(t *testing.T, f *localRepositoryFixture) string {
			replaceFixtureTags(t, f, "release")
			return "main"
		}, want: "pseudo"},
		{name: "noncanonical short SemVer Tag is ignored by main", prepare: func(t *testing.T, f *localRepositoryFixture) string {
