 ...
 * [OUTPUT]: Specifies the stash package behavior covered by with_redis_test.go.
...
func TestWithRedisLock(t *testing.T) {
	...
	password := os.Getenv("SKILLSGO_HUB_REDIS_PASSWORD")
	if len(endpoint) == 0 {
		t.SkipNow()
	}
...
func TestWithRedisLockWithPassword(t *testing.T) {
	...
	password := os.Getenv("SKILLSGO_HUB_PROTECTED_REDIS_PASSWORD")
	if len(endpoint) == 0 {
		t.SkipNow()
	}
	strg, err := mem.NewStorage()
