...
function stopProcesses () {
  Get-Process -Name hub* -ErrorAction SilentlyContinue | Stop-Process -Force
}
...
try {
  ...
  Push-Location $(Join-Path $repoDir cmd | Join-Path -ChildPath hub)
  ## just in case something is still running
  stopProcesses
  & go build -mod=vendor
  Start-Process -NoNewWindow .\hub.exe
