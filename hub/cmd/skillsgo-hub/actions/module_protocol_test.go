...
func TestDuplicateNamePublicationPreservesDistinctPathMembers(t *testing.T) {
	...
	_, err = publisher.Materialize(t.Context(), repository, version)
	require.NoError(t, err)
	members, membersErr := metadata.VersionSkills(t.Context(), repository, version)
	require.NoError(t, membersErr)
	require.Len(t, members, 2)
	require.Equal(t, []string{"skills/one", "skills/two"}, []string{members[0].Path, members[1].Path})
	_, storageErr := backend.Info(t.Context(), repository, version)
...
func repositoryTestMember(t *testing.T, memberPath string, archive, info []byte) skill.RepositoryMember {
	...
	return skill.RepositoryMember{Name: manifest.Name, Path: memberPath, TreeSHA: identity.TreeSHA, Manifest: manifest, Content: archive}
}
...
func TestUnknownRepositoryExactInfoPublishesOneSnapshotAndThenUsesCache(t *testing.T) {
	...
	serveFiber(t, router, detailRecorder, httptest.NewRequest(http.MethodGet, "/api/v1/"+repository+"/versions/"+version+"/skills?path=skills/find-skills", nil))
...
func TestRepositoryWithoutRootSkillPublishesCompleteRepositoryArtifact(t *testing.T) {
	...
	serveFiber(t, router, zipRecorder, httptest.NewRequest(http.MethodGet, "/api/v1/"+repository+"/versions/"+version+".zip", nil))
...
func TestAnonymousModulePublicationReturnsStableOverloadAndReleasesCapacity(t *testing.T) {
	...
	serveFiber(t, router, overloaded, httptest.NewRequest(http.MethodGet, "/api/v1/"+repositories[8]+"/versions/"+version+"", nil))
	...
	serveFiber(t, router, retry, httptest.NewRequest(http.MethodGet, "/api/v1/"+repositories[8]+"/versions/"+version+"", nil))
...
func TestRepositoryHistoryPreservesExactMemberSetsWithoutSkillArtifactRoutes(t *testing.T) {
	...
	serveFiber(t, router, recorder, httptest.NewRequest(http.MethodGet, "/api/v1/"+legacyMemberRoute+"/versions/v1.0.0.zip", nil))
