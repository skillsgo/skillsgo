...
- `catalog_api.go`, `catalog_api_test.go`: expose `GET /api/v1/skills/find` with zero-based page/per-page requests and greedy one-extra-row has-more responses, dedicated `POST /api/v1/skills/find-candidates`, ordered batch Skill-card hydration, and `POST /api/v1/skills/check-update` while retaining correlated private diagnostics for safe public failures.
