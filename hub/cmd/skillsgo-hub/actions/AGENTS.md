...
- `catalog_api.go`, `catalog_api_test.go`: expose stable public search, ordered batch Skill-card hydration, shared Protocol batch-update DTOs, immutable artifact detail, and pagination while retaining correlated private diagnostics for safe public failures.
...
- `repository_publisher.go`: validates and stores one complete Repository Artifact, creates typed immutable Repository/Skill member Info, coordinates bounded and negative-cached source work, rolls back failed storage commits, and atomically exposes Catalog membership.
