...
- `package_backfill.go`, `package_backfill_test.go`: validate and expose bounded `/api/v1/admin/package-backfills` batches, persist one independent Run per Package, prepare and close one leased source session per River job, execute at most twenty highest semantic Tags or no-Tag recent default-branch pseudo-versions through one chunked Publisher session, and retain bounded diagnostics.
