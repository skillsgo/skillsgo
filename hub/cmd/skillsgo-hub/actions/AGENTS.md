...
- `background_tasks.go`, `background_tasks_test.go`: define and verify first-class River JobArgs, stable observable kinds, source/maintenance workload placement, payload validation, uniqueness fields, retry limits, and domain-handler adapters for Source Repository metadata, Module prewarm, and translation.
...
- `catalog_api.go`, `catalog_api_test.go`: expose `GET /api/v1/skills/find` with zero-based page/per-page requests and greedy one-extra-row has-more responses, dedicated `POST /api/v1/skills/find-candidates`, exact Module Path plus Skill Path ordered batch-card hydration, and `POST /api/v1/skills/check-update` while retaining correlated private diagnostics for safe public failures.
...
- `module_protocol.go`: serves persisted Module Info and ZIP resources on canonical Module Paths, triggers exact cold publication, rejects nested Skill artifact coordinates, and reports publication-cache decisions.
- `module_skill.go`: serves one exact Skill path under a canonical Module Version, resolves movable queries to immutable versions, and reads SKILL.md directly from immutable sidecar storage.
- `module_publisher.go`: validates one complete Module Artifact, prepares typed immutable Module Info and direct Skill content sidecars, and coordinates bounded and negative-cached Source Repository work.
- `module_publication_commit.go`: owns retry-safe immutable Artifact then Skill-content residency followed by atomic Catalog visibility, retaining failed-publication orphans for safe later residency GC instead of racing with concurrent publishers.
- `module_version_query.go`, `module_version_query_test.go`: resolve typed movable Version Queries once into immutable Module Version Records through the Module-scoped query API.
- `module_backfill.go`, `module_backfill_test.go`, `module_backfill_postgres_integration_test.go`: validate and expose bounded `/api/v1/admin/module-backfills` batches, persist one independent Run per Module, execute deterministic semantic-version history through River, retain bounded diagnostics, and verify transactional restart/multi-instance behavior.
