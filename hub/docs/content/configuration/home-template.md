...
As of v0.14.0 Athens ships with a default, minimal HTML home page that advises users on how to connect to the proxy. It relies on the users request Host header (on HTTP 1.1) or the Authority header (on HTTP 2) as well as whether the request was over TLS to advise
...
Do so by configuring `HomeTemplatePath` via the config or `SKILLSGO_HUB_HOME_TEMPLATE_PATH` to a location on disk with a Go HTML template or placing a template file at `/var/lib/athens/home.html`.
...
<body>
	...
	<pre>GET {{ .Host }}/mod/github.com/acidburn/htp/@v/v1.0.0.mod</pre>
