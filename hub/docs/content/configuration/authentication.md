...
3.  Once we've properly authenticated we want to share the .subversion directory with the Athens proxy server in order to reuse those credentials. Below we're setting it as a volume on our proxy container.
    ...
    **Bash**
    ...
    ```bash
    export SKILLSGO_HUB_STORAGE=~/athens-storage
    export SKILLSGO_HUB_SVN=~/.subversion
    mkdir -p $SKILLSGO_HUB_STORAGE
    docker run -d -v $SKILLSGO_HUB_STORAGE:/var/lib/athens \
      -v $SKILLSGO_HUB_SVN:/root/.subversion \
      -e SKILLSGO_HUB_DISK_STORAGE_ROOT=/var/lib/athens \
      -e SKILLSGO_HUB_STORAGE_TYPE=disk \
      --name athens-proxy \
      --restart always \
      -p 3000:3000 \
    ...
    **PowerShell**
    ...
    ```PowerShell
    $env:SKILLSGO_HUB_STORAGE = "$(Join-Path $pwd athens-storage)"
    $env:SKILLSGO_HUB_SVN = "$(Join-Path $pwd .subversion)"
    md -Path $env:SKILLSGO_HUB_STORAGE
    docker run -d -v "$($env:SKILLSGO_HUB_STORAGE):/var/lib/athens" `
      -v "$($env:SKILLSGO_HUB_SVN):/root/.subversion" `
      -e SKILLSGO_HUB_DISK_STORAGE_ROOT=/var/lib/athens `
      -e SKILLSGO_HUB_STORAGE_TYPE=disk `
      --name athens-proxy `
      --restart always `
...
5. Once we've properly setup our authentication we want to share the bazaar configuration directory with the Athens proxy server in order to reuse those credentials. Below we're setting it as a volume on our proxy container.
   ...
   **Bash**
   ...
   ```bash
   export SKILLSGO_HUB_STORAGE=~/athens-storage
   export SKILLSGO_HUB_BZR=~/.bazaar
   mkdir -p $SKILLSGO_HUB_STORAGE
   docker run -d -v $SKILLSGO_HUB_STORAGE:/var/lib/athens \
     -v $SKILLSGO_HUB_BZR:/root/.bazaar \
     -e SKILLSGO_HUB_DISK_STORAGE_ROOT=/var/lib/athens \
     -e SKILLSGO_HUB_STORAGE_TYPE=disk \
     --name athens-proxy \
     --restart always \
   ...
   **PowerShell**
   ...
   ```PowerShell
   $env:SKILLSGO_HUB_STORAGE = "$(Join-Path $pwd athens-storage)"
   $env:SKILLSGO_HUB_BZR = "$(Join-Path $pwd .bazaar)"
   md -Path $env:SKILLSGO_HUB_STORAGE
   docker run -d -v "$($env:SKILLSGO_HUB_STORAGE):/var/lib/athens" `
     -v "$($env:SKILLSGO_HUB_BZR):/root/.bazaar" `
     -e SKILLSGO_HUB_DISK_STORAGE_ROOT=/var/lib/athens `
     -e SKILLSGO_HUB_STORAGE_TYPE=disk `
     --name athens-proxy `
     --restart always `
...
```bash
$ mkdir -p storage
$ SKILLSGO_HUB_STORAGE=storage
$ docker run --rm -d \
    -v "$PWD/$SKILLSGO_HUB_STORAGE:/var/lib/athens" \
    -v "$PWD/gitconfig/.gitconfig:/root/.gitconfig" \
    -v "$PWD/ssh-keys:/root/.ssh" \
    -e SKILLSGO_HUB_DISK_STORAGE_ROOT=/var/lib/athens -e SKILLSGO_HUB_STORAGE_TYPE=disk --name athens-proxy -p 3000:3000 gomods/athens:canary
```
