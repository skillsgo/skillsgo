...
Then we need to set the `SKILLSGO_HUB_STORAGE_TYPE` and `SKILLSGO_HUB_DISK_STORAGE_ROOT` environment variables when we run the Docker container.

**Bash**
```bash
export SKILLSGO_HUB_STORAGE=$HOME/athens-storage
...
**PowerShell**
```PowerShell
$env:SKILLSGO_HUB_STORAGE = "$(Join-Path $HOME athens-storage)"
