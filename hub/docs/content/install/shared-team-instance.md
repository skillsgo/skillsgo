...
In order to run Athens with disk storage, you will next need to identify where you would like to persist modules. In the example below, we will create a new directory named `athens-storage` in our current directory.  Now you are ready to run Athens with disk storage enabled. To enable disk storage, you need to set the `SKILLSGO_HUB_STORAGE_TYPE` and `SKILLSGO_HUB_DISK_STORAGE_ROOT` environment variables when you run the Docker container.
...
**Bash**
```console
$ ls -lr $SKILLSGO_HUB_STORAGE/github.com/athens-artifacts/samplelib/v1.0.0/
total 24
...
**PowerShell**
```console
$ dir $env:SKILLSGO_HUB_STORAGE\github.com\athens-artifacts\samplelib\v1.0.0\
