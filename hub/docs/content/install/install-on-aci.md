...
- `AZURE_SKILLSGO_HUB_RESOURCE_GROUP` - The [Azure Resource Group](https://www.petri.com/what-are-microsoft-azure-resource-groups) to install the container in. You need to already have one of these before installing Athens
    - See [here](https://cda.ms/KS) for details on how to create a resource group
- `AZURE_SKILLSGO_HUB_CONTAINER_NAME` - The name of the container. This should be alphanumeric and you can have `-` and `_` characters
- `LOCATION` - The [Azure region](https://cda.ms/KT) to install the container in. See the previous link for an exhaustive list, but here's a useful cheat sheet that you can use immediately, without reading any docs:
    ...
    - Europe: `westeurope`
    - Asia: `southeastasia` 
- `AZURE_SKILLSGO_HUB_DNS_NAME` - The DNS name to assign to the container. It has to be globally unique inside of the region you set (`LOCATION`)
...
```console
az container create \
-g "${AZURE_SKILLSGO_HUB_RESOURCE_GROUP}" \
-n "${AZURE_SKILLSGO_HUB_CONTAINER_NAME}-${LOCATION}" \
--image gomods/athens:v0.3.0 \
-e "SKILLSGO_HUB_STORAGE_TYPE=disk" "SKILLSGO_HUB_DISK_STORAGE_ROOT=/var/lib/athens" \
--ip-address=Public \
--dns-name="${AZURE_SKILLSGO_HUB_DNS_NAME}" \
--ports="3000" \
--location=${LOCATION}
```

Once you've created the ACI container, you'll see a JSON blob that includes the public IP address of the container. You'll also see the [fully qualified domain name](https://en.wikipedia.org/wiki/Fully_qualified_domain_name) (FQDN) of the running container (it will be prefixed by `AZURE_SKILLSGO_HUB_DNS_NAME`).

### Installing with the MongoDB Storage Driver

First, make sure you have the following environment variable set up:

- `AZURE_SKILLSGO_HUB_MONGO_URL` - The MongoDB connection string. For example: `mongodb://username:password@mongo.server.com/?ssl=true`
...
-n "${AZURE_SKILLSGO_HUB_CONTAINER_NAME}-${LOCATION}" \
--image gomods/athens:v0.3.0 \
-e "SKILLSGO_HUB_STORAGE_TYPE=mongo" "SKILLSGO_HUB_MONGO_STORAGE_URL=${AZURE_SKILLSGO_HUB_MONGO_URL}" \
--ip-address=Public \
--dns-name="${AZURE_SKILLSGO_HUB_DNS_NAME}" \
--ports="3000" \
--location=${LOCATION}
