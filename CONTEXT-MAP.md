...
SkillsGo is a monorepo containing a desktop App, a local CLI, and a public Registry. Each context owns a separate model while sharing the Skill identity and artifact protocol.
...
- [Registry](registry/CONTEXT.md) — resolves public Skill sources into immutable artifacts and serves search, ranking, metadata, download, and install-event APIs.

## Relationships

- **App → Registry**: reads search, ranking, detail, risk, and immutable artifact metadata.
- **App → CLI**: invokes the bundled CLI through stable JSON contracts for local discovery and mutations.
- **CLI → Registry**: resolves versions, downloads verified artifacts, and optionally reports anonymous install events.
