...
ADR-0010 replaces the rejected per-Skill artifact model with one immutable artifact, version, ZIP, Sum, download, lock, and Module Store unit per Repository Version. ADR-0011 identifies selectable members by Repository ID plus canonical Skill Name and keeps Skill Path as immutable release metadata.
