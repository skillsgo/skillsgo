...
5. The protected release workflow uploads immutable objects, reads them back through the public CDN, creates the immutable GitHub Release, and publishes `/cli/stable/manifest.*` last.
...
- CDN compromise alone cannot forge trusted release metadata or redirect clients to another Origin, while signing-key compromise remains a protected-environment incident requiring key rotation and a client trust-root release.
- Production publication requires CLI-scoped object-storage credentials in the `cli-release` Environment. Missing credentials fail the release rather than degrading to GitHub-only update discovery.
