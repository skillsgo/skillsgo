...
1. Standalone CLI releases use `cli/vX.Y.Z` tags and five Tier-1 archives: macOS arm64/amd64, Linux arm64/amd64, and Windows amd64. Each release also assembles the unscoped npm `skillsgo` launcher with five optional native packages and a generated Homebrew Formula for macOS/Linux.
...
8. GitHub Release is the public archive and fallback download location. The release workflow attaches npm tarballs and the generated Formula as immutable release assets. Optional publisher jobs can publish the tarballs to npm and the Formula to the Homebrew tap when their repository-level gates and credentials are enabled. Installed clients do not depend on the GitHub Release API.
...
- Homebrew, npm, npx, and `go install` integrations consume the release contract or direct users to their native upgrade mechanism. WinGet remains a follow-up publisher because its manifest lives outside this repository. Package-manager publication is explicitly gated and cannot weaken the signed update Origin.
