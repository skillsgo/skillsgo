...
Execution revalidates the planned External path and selected immutable coordinate but does not compare the External bytes with the selected member. It installs the authenticated complete Repository Artifact through the ordinary Dependency, Lock, Scope Module Store, and Repository Projection transaction. Only after that transaction commits does it remove the External directory from its active Agent path. A changed, missing, conflicting, or otherwise non-executable row is skipped independently through the existing takeover result flow.
