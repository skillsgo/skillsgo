...
func Replace(entry *store.Entry, previous []Installation, targets []Target) error {
	...
	for _, target := range targets {
		...
		action, err := replaceTarget(entry.Artifact, target, old, tracked)
		if err != nil {
...
func replaceTarget(artifact string, target Target, previous Installation, tracked bool) (switchAction, error) {
	path := filepath.Clean(target.Path)
	...
	if target.Mode != previous.Target.Mode {
