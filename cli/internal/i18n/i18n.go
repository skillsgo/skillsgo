...
var messages = map[string][2]string{
	...
	"cache.warm.short":                      {"Download and verify one exact immutable Skill without installing it", "下载并校验一个精确不可变 Skill，但不执行安装"},
	"cache.warm.error.coordinate":           {"cache warm requires one public Skill with an exact immutable version", "cache warm 需要一个带精确不可变版本的公开 Skill"},
	"cache.warm.success":                    {"Cached %s@%s without installing targets\n", "已缓存 %s@%s，未安装任何目标\n"},
	...
	"takeover.error.scope":                  {"Batch Takeover requires --user or at least one --project", "批量接管需要使用 --user 或至少一个 --project 指定范围"},
	...
	"takeover.flag.user":                    {"Include the user installation scope", "包含用户级安装范围"},
	...
	"inventory.title":                       {"Skill Inventory", "Skill 清单"},
	...
	"operation.store":                       {"Add artifact to Store", "写入 Store"},
	...
	"operation.export":                      {"Exporting Local Skill", "正在导出本地 Skill"},
	"result.update":                         {"Update Results", "更新结果"},
	"result.remove":                         {"Removal Complete", "移除完成"},
	"result.export":                         {"Export Complete", "导出完成"},
	...
	"list.title.global":                     {"Global Skills", "全局 Skills"},
	"list.title.project":                    {"Project Skills", "项目 Skills"},
	"list.section.hub":                      {"Managed", "已管理"},
	"list.section.local":                    {"Local", "本地"},
	"list.section.external":                 {"External", "外部安装"},
