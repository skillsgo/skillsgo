...
- `workspace_yaml.go`: owns strict `skillsgo.yaml` Repository dependency intent, strict `skillsgo-lock.yaml` integrity, nearest Workspace discovery, atomic paired loading with journal recovery, exact pair validation, deterministic normalization, and paired crash-recoverable publication.
