...
- `project_registry.go`, `project_discovery.go`, `project_registry_test.go`: expose and specify one-time recent-Agent Workspace bootstrap, explicit registration/removal, and stable machine listing over the `projects` section of CLI-owned user configuration.
