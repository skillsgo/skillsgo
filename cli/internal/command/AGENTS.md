...
- `product_reads.go`: exposes top-level Skill `find`, same-Origin `rankings`, `hub check`, user-triggered `hub check-update`, and strict file/stdin source-language `hub find-candidates` reads; single Find classifies all explicit Package aliases through the CLI Source parser, resolves the selected Package query once, preserves its immutable version through product Find, and leaves keyword and candidate Find Catalog-only.
...
- `product_reads_test.go`: specifies single and batch Find, immutable explicit-source resolution, ordered batch hydration, user-triggered Package update checks, and grouped Hub service inspection through Execute.
