...
- `product_reads.go`: exposes top-level Skill `find`, same-Origin `rankings`, `hub info`, `hub check`, user-triggered `hub check-update`, and strict file/stdin source-language `hub find-candidates` reads; single Find classifies all explicit Package aliases through the CLI Source parser, resolves the selected Package query once, preserves its immutable version through product Find, and leaves keyword and candidate Find Catalog-only.
- `project_registry.go`, `project_registry_test.go`: orchestrate and specify one-time recent-Agent Workspace bootstrap, explicit registration/removal, and stable machine listing over the `projects` section of CLI-owned user configuration while Agent-owned adapters provide discovery evidence.
- `product_reads_test.go`: specifies single and batch Find, immutable explicit-source resolution, ordered batch hydration, Hub capability discovery, user-triggered Package update checks, and grouped Hub service inspection through Execute.
...
- `list.go`: owns the sole installed-Skill listing command, supplies the read-through Package Provider for exact metadata cache reconstruction, collects best-effort per-Agent usage evidence, and adapts Package-managed/external inventory v8 with optional lock-backed Adoption Package hints into stable JSON plus path-rich Human output.
