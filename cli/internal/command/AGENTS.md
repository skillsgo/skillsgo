...
- `list.go`: owns the sole installed-Skill listing command, supplies the read-through Package Provider for exact metadata cache reconstruction, and adapts backward-compatible Package-managed/external inventory v7 with optional lock-backed Adoption Package hints into stable JSON plus path-rich Human output.
