...
- `package_update.go`: previews or confirms Package-granularity non-decreasing updates for one Scope or every Managed Scope, deduplicates Catalog resolution across Scope targets, binds mutations to current YAML/Lock state, verifies the existing Package Store/Projections, removes unavailable selectors, and reports complete per-Package outcomes.
