...
- `client.go`: delegates version selectors to the Hub `/mod` surface, consumes flat self-contained Repository/Skill Info, reads provider-neutral `/api/v1` product metadata for App-ready cards, downloads verified ZIP responses with optional byte progress, resolves exact content matches, and exposes typed HTTP failures from a configured SkillsGo Hub.
