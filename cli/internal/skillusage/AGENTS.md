...
- `agentsview.go`: is the active and sole Library usage source; it owns the private `~/.skillsgo/sessions/sessions.db` archive lifecycle, coordinates one non-blocking process-local AgentsView sync, protects SQLite sidecars, immediately reuses a valid existing `CallCount` snapshot while refreshing it, publishes an explicit pending state only before the first usable snapshot, emits throttled process-wide phase-local progress with reconnect snapshots, and emits coalesced analytics revisions after successful snapshots.
- `agentsview_test.go`: specifies immediate first-archive pending results, stale-while-revalidate snapshot availability, eventual isolated archive creation, private directory permissions, the stable empty per-Agent usage shape, monotonic analytics invalidations, and coalesced progress subscriptions.
