...
- `repository.go`: reconciles strict YAML/Lock dependencies, scoped immutable Repository Info, verified Package Store, deterministic Projections, selected Skill members, and Local Modifications without Hub access.
...
This module owns read-only Library reconciliation and stable inventory domain records. It may inspect only Global/project Repository declarations and locks, immutable scoped Repository Info, Package Store/Projection state, and known Agent directories; it must not mutate Skill content, contact a Hub, or serialize localized CLI output.
