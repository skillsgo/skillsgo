...
| `api/` | Public CLI-to-Hub JSON DTOs, including canonical zero-based pagination, Package Version collections, single/batch Find documents, current Package Publications, Package-level Sum, immutable Package size and Artifact Repository identity, canonical Package-member coordinates, Skill path metadata, schema constants, and statuses. |
