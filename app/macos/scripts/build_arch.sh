...
# [OUTPUT]: Produces one fully architecture-, version-, update-source-, and bundle-signed Release SkillsGo.app in an isolated DerivedData directory.
...
readonly code_signing_allowed="${SKILLSGO_MACOS_CODE_SIGNING_ALLOWED:-YES}"
...
if [[ -n "${SKILLSGO_APP_UPDATE_URL:-}" ]]; then
  ...
  case "${architecture}" in
    arm64) readonly app_update_channel="osx-arm64" ;;
    x86_64) readonly app_update_channel="osx-x64" ;;
  esac
  flutter_build_args+=(--dart-define "SKILLSGO_APP_UPDATE_CHANNEL=${app_update_channel}")
fi
