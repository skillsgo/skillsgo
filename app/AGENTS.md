...
- The App invokes the bundled SkillsGo CLI through one typed long-lived CLI Server adapter and must not call public Hub APIs directly. In Cloud mode it may call only the Cloud origin declared by `skillsgo hub info` for Cloud-composed ranking reads.
