/*
 * [INPUT]: Depends on the App updater's official platform-specific production defaults, production overrides, and unsigned rehearsal source/channel parsers.
 * [OUTPUT]: Proves ordinary builds use the official CDN, production overrides accept only clean HTTPS feeds, and the CI escape hatch accepts only explicit loopback HTTP feeds and safe Velopack channels.
...
void main() {
  test(
    'production update source defaults to the official platform channel',
    () {
      final channel = appUpdateProductionChannel();
      expect(channel, isNotNull);
      expect(
        appUpdateProductionSource(),
        Uri.parse('https://cdn.skillsgo.ai/app/$channel/'),
      );
    },
  );

  test('an explicitly empty production update source disables updates', () {
    expect(appUpdateProductionSource(''), isNull);
    expect(appUpdateProductionSource('   '), isNull);
  });

  test('production update channel is explicit and validated', () {
    expect(appUpdateProductionChannel('osx-arm64'), 'osx-arm64');
    expect(appUpdateProductionChannel(''), isNull);
    expect(
      () => appUpdateProductionChannel('../osx-arm64'),
