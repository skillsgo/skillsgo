 ...
 * [OUTPUT]: Specifies Remote-detail restoration, direct selected-version installation, next-frame task feedback with non-blocking selector dismissal, risk, recovery, and target behavior.
...
void main() {
  ...
  testWidgets(
    'installation submit moves into the task stack in the next frame',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1400, 900));
      final pending = Completer<CommandResult>();
      final gateway = FakeSkillsGateway(
        installed: false,
        installCompleter: pending,
      );
      await tester.pumpWidget(SkillsGoApp(gateway: gateway));
      await tester.pumpAndSettle();
      await tester.enterText(searchInput(), 'pending-install');
      await tester.testTextInput.receiveAction(TextInputAction.search);
      await tester.pumpAndSettle();
      await tester.tap(find.text('Flutter Pro'));
      await tester.pumpAndSettle();
      ...
      final submit = find.byKey(const ValueKey('install-location-submit'));
      await tester.tap(submit);
      await tester.pump();

      final taskStack = find.byKey(const Key('installation-task-stack'));
      expect(taskStack, findsOneWidget);
      expect(
        find.descendant(
          of: taskStack,
      ...
      expect(find.text('Flutter Pro'), findsWidgets);
      ...
      pending.complete(
        const CommandResult(
          ...
          output: ProcessOutput(exitCode: 0, stdout: '{}', stderr: ''),
        ),
      );
      await tester.pumpAndSettle();
