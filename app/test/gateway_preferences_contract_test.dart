 ...
 * [OUTPUT]: Specifies appearance including persistent first-run theme-color and wallpaper selection, language, reminder, macOS protected-folder-sensitive local-scan acceptance/deferral and recovery, one-time adoption-introduction, the single Hub Origin, onboarding, Agent-session-bootstrapped Added Projects, offline local-management, risk, storage, and diagnostics contracts.
...
class _ManagedProjectsRunner implements ProcessRunner {
  ...
  Future<ProcessOutput> run(
    ...
    if (arguments.length >= 2 && arguments.first == 'project') {
      ...
      if (action == 'bootstrap') {
        return _document('project-bootstrap', projects.values.toList());
      }
    }
    return fallback?.run(
          executable,
...
void main() {
  ...
  test('local scan notice acceptance persists once', () async {
    final gateway = DesktopSkillsGateway();

    expect(
      await gateway.loadLocalScanNoticeDecision(),
      LocalScanNoticeDecision.unseen,
    );
    await gateway.saveLocalScanNoticeDecision(LocalScanNoticeDecision.accepted);
    expect(
      await DesktopSkillsGateway().loadLocalScanNoticeDecision(),
      LocalScanNoticeDecision.accepted,
    );
  });

  test('local scan notice deferral persists until acceptance', () async {
    final gateway = DesktopSkillsGateway();

    await gateway.saveLocalScanNoticeDecision(LocalScanNoticeDecision.deferred);
    expect(
      await DesktopSkillsGateway().loadLocalScanNoticeDecision(),
  ...
  test(
    'local scan notice is required only for macOS protected project paths',
    () async {
      final home = Platform.environment['HOME']!;
      final protectedRunner = _ManagedProjectsRunner();
      protectedRunner.projects['$home/Documents/workspace'] = (
        name: 'workspace',
        root: '$home/Documents/workspace',
      );
      final protectedGateway = DesktopSkillsGateway(
        processRunner: protectedRunner,
        initialCliPath: '/bin/skillsgo',
      );

      expect(await protectedGateway.loadLocalScanNoticePaths(), [
        '$home/Documents/workspace',
      ]);
      expect(protectedRunner.projectActions, ['list']);

      final ordinaryRunner = _ManagedProjectsRunner();
      ordinaryRunner.projects['/opt/workspace'] = (
        name: 'workspace',
        root: '/opt/workspace',
      );
      final ordinaryGateway = DesktopSkillsGateway(
        processRunner: ordinaryRunner,
        initialCliPath: '/bin/skillsgo',
      );

      expect(await ordinaryGateway.loadLocalScanNoticePaths(), isEmpty);
      expect(ordinaryRunner.projectActions, ['list']);
    },
    skip: !Platform.isMacOS,
  );

  test(
    'Added Projects bootstrap from local Agent sessions before loading',
    () async {
      final runner = _ManagedProjectsRunner();
      runner.projects['/work/recent'] = (name: 'recent', root: '/work/recent');
      final gateway = DesktopSkillsGateway(
      ...
      expect(projects.single.name, 'recent');
      expect(runner.projectActions.first, 'bootstrap');
    },
  );
    ...
    final runner = FakeCliServerRunner()
      ..responses.add(
        const ProcessOutput(
          exitCode: 0,
          stdout:
              '{"schemaVersion":1,"product":"skillsgo","version":"test","appProtocolVersion":20,"os":"darwin","architecture":"arm64"}',
          stderr: '',
        ),
      )
      ..serverResponses.add(
        const ProcessOutput(
          exitCode: 0,
          stdout:
              '{"schemaVersion":2,"product":"skillsgo","version":"test","appProtocolVersion":20,"os":"darwin","architecture":"arm64","agents":[{"id":"codex","displayName":"Codex","installed":true,"supportedScopes":["global"],"globalTarget":{"path":"/Users/test/.codex/skills","exists":true}}]}',
          stderr: '',
        ),
      );
    final gateway = DesktopSkillsGateway(
              ...
              'entries': [
                {
                  ...
                  'usage': {'hits45Days': 0, 'hits90Days': 0},
                  'visibility': <Object>[],
                  'targets': [
                    {
                      'scope': 'global',
