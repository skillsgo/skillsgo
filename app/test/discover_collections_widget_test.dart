 ...
 * [OUTPUT]: Specifies Discover search, source collections, Package update-check and stable install entries, leaderboard layout, pagination, and refresh behavior.
...
void main() {
  ...
  testWidgets('Git URLs and shorthand keep Skill cards inside source context', (
    ...
    expect(find.byKey(const Key('package-check-update')), findsOneWidget);
    expect(find.text('Check updates'), findsOneWidget);
    await tester.tap(find.byKey(const Key('package-check-update')));
    await tester.pumpAndSettle();
    expect(gateway.packageUpdateChecks, ['github.com/example/skills']);
    ...
    final packageCard = tester.getRect(
      find.byKey(const Key('package-summary-card')),
    );
    await tester.tapAt(Offset(packageCard.left + 8, packageCard.bottom - 8));
    await tester.pumpAndSettle();
