 ...
 * [OUTPUT]: Specifies Cross-destination state restoration, navigation concurrency, Library keyboard traversal, focus, scrolling, and accessibility.
...
void main() {
  testWidgets('Library keyboard traversal reaches governance and inventory', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1200, 800));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    const skill = InstalledSkill(
      inventoryKey: 'keyboard-skill',
      name: 'keyboard-skill',
    ...
    await tester.pumpWidget(
      SkillsGoApp(gateway: FakeSkillsGateway(libraryEntries: const [skill])),
    );
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const Key('primary-destination-library')));
    await tester.pumpAndSettle();

    bool hasFocus(Key key) {
      ...
      return find
              .ancestor(of: focused, matching: find.byKey(key))
              .evaluate()
              .isNotEmpty ||
          context.widget.key == key;
    }

    Future<void> tabUntil(Key key) async {
      for (var step = 0; step < 30 && !hasFocus(key); step += 1) {
        await tester.sendKeyEvent(LogicalKeyboardKey.tab);
        await tester.pump();
      }
      expect(hasFocus(key), isTrue, reason: '$key must be keyboard reachable');
    }

    await tester.tap(librarySearchInput());
    await tester.pump();
    await tabUntil(const Key('library-filter-trigger'));
    await tabUntil(const Key('library-content-all-skills'));
    await tabUntil(const ValueKey('library-sort-hits45Days'));
    await tabUntil(const ValueKey('library-row-keyboard-skill'));

    await tester.tap(find.byKey(const Key('library-content-needs-attention')));
    await tester.pumpAndSettle();
    await tabUntil(const Key('library-attention-unused'));
    await tabUntil(const Key('library-attention-other-installation'));
    await tabUntil(const Key('library-attention-updates'));
    await tester.tap(find.byKey(const Key('library-content-all-skills')));
    await tester.pumpAndSettle();

    await tester.tap(
      find.byKey(const ValueKey('library-select-keyboard-skill')),
    );
    await tester.pumpAndSettle();
    await tester.tap(librarySearchInput());
    await tester.pump();
    await tabUntil(const Key('library-clear-selection'));
