 ...
 * [OUTPUT]: Specifies deterministic optionally delayed queued one-shot and delegating or explicit CLI Server adapters with recorded executable, structured arguments, and per-call stdin documents.
...
class FakeProcessRunner implements ProcessRunner {
  ...
  final responseDelays = <Duration>[];

  @override
  Future<CliServerSession> startCliServer(String executable) async =>
      _DelegatingFakeCliServerSession(this, executable);
  ...
  Future<ProcessOutput> run(
    ...
    final delay = responseDelays.isNotEmpty
        ? responseDelays.removeAt(0)
        : Duration.zero;
...
class _DelegatingFakeCliServerSession implements CliServerSession {
  _DelegatingFakeCliServerSession(this.runner, this.executable);
