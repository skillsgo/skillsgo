 ...
 * [OUTPUT]: Specifies startup presentation readiness, Locale, folder theme, appearance mode, wallpaper, Bloom preset behavior, and Library readability across theme/chroma extremes.
...
void main() {
  ...
  testWidgets('Library remains readable across theme and chroma extremes', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1200, 800));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    const skill = InstalledSkill(
      inventoryKey: 'theme-skill',
      name: 'theme-skill',
      path: '/tmp/theme-skill',
    ...
    for (final mode in [AppThemeMode.light, AppThemeMode.dark]) {
      for (final seed in ['#000000', '#FF00FF']) {
        await tester.pumpWidget(
          SkillsGoApp(
            gateway: FakeSkillsGateway(
              themeMode: mode,
              folderTheme: seed,
              libraryEntries: const [skill],
            ),
          ),
        );
        await tester.pumpAndSettle();
        ...
        expect(
          ...
          greaterThanOrEqualTo(4.5),
          reason: '${mode.name} $seed Library content contrast',
        );
        expect(find.text('theme-skill'), findsOneWidget);
