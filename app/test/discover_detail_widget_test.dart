 ...
 * [OUTPUT]: Specifies Discover card installation entry points, remote detail evidence, Package navigation and update checks, inline translation switching, motion, and theme behavior.
...
void main() {
  testWidgets('Skill detail opens its immutable Package detail', (
    tester,
  ) async {
    ...
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const Key('detail-open-package')));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('package-detail-scroll')), findsOneWidget);
    expect(find.text('example/skills'), findsOneWidget);
    expect(
      find.ancestor(
        of: find.byKey(const Key('package-detail-scroll')),
        matching: find.byKey(const Key('discover-body-stack')),
      ),
      findsOneWidget,
    );

    await tester.tap(find.byKey(const Key('package-detail-back')));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('detail-open-package')), findsOneWidget);
  });
  ...
  testWidgets('detail exposes canonical Package Version Skill metadata', (
    ...
    expect(find.byKey(const Key('detail-check-update')), findsOneWidget);
    expect(find.text('Check updates'), findsOneWidget);
    await tester.tap(find.byKey(const Key('detail-check-update')));
    await tester.pumpAndSettle();
