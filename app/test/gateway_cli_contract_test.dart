 ...
 * [OUTPUT]: Specifies bundled CLI startup handshake, platform compatibility, developer override, analytics invalidation forwarding, non-destructive concurrent detection, safe-read transport recovery, and revalidation contracts.
...
void main() {
  ...
  test('CLI analytics invalidations reach the gateway stream', () async {
    final runner = _AnalyticsCliServerRunner();
    final gateway = DesktopSkillsGateway(
      processRunner: runner,
      initialCliPath: '/bundle/skillsgo',
    );
    final nextRevision = gateway.watchAnalyticsInvalidations().first;

    await gateway.inspectAgents();
    runner.session.events.add(const CliServerAnalyticsInvalidation(42));

    expect(await nextRevision, 42);
  });

  test('CLI analytics progress reaches the gateway stream', () async {
    final runner = _AnalyticsCliServerRunner();
    final gateway = DesktopSkillsGateway(
      processRunner: runner,
      initialCliPath: '/bundle/skillsgo',
    );
    final nextProgress = gateway.watchAnalyticsProgress().first;

    await gateway.inspectAgents();
    runner.session.progress.add(
      const CliServerAnalyticsProgress(
    ...
    final value = await nextProgress;
    expect(value.revision, 7);
    expect(value.fraction, .6);
  });
}
...
class _AnalyticsCliServerRunner extends FakeProcessRunner
    implements CliServerRunner {
  late final _AnalyticsCliServerSession session;

  @override
  Future<CliServerSession> startCliServer(String executable) async =>
      session = _AnalyticsCliServerSession();
}

class _AnalyticsCliServerSession
    implements CliServerSession, CliServerAnalyticsEventSource {
  final events = StreamController<CliServerAnalyticsInvalidation>();
  final progress = StreamController<CliServerAnalyticsProgress>();
