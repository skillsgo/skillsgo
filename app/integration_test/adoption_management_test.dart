/*
 * [INPUT]: Depends on the rendered App with bounded startup pumping, bundled CLI, JourneyRuntime filesystem/Hub/schema isolation and CLI-backed Project registration, supported skills.sh locks, and the public versioned Repository fixture.
 * [OUTPUT]: Exports and registers a reusable Journey that verifies location-scoped existing-Skill adoption, Global and Project persistence, post-success rescans, managed actions, and restoration of preserved original Skill bytes.
...
void registerAdoptionManagementJourney() {
  testWidgets(
    ...
    runAdoptionManagementJourney,
    timeout: const Timeout(Duration(minutes: 3)),
  );
}

Future<void> runAdoptionManagementJourney(WidgetTester tester) async {
  ...
  try {
    ...
    final globalTarget = Directory('$sandbox/test-agent/skills/user-existing');
    ...
    File('${projectTarget.path}/SKILL.md').writeAsBytesSync(projectSkillBytes);
    ...
    final needsAttention = find.byKey(
      const Key('library-content-needs-attention'),
    );
    await _pumpUntil(tester, needsAttention);
    await tester.tap(needsAttention);
    final otherInstallation = find.byKey(
      const Key('library-attention-other-installation'),
    );
    await _pumpUntil(tester, otherInstallation);
    await tester.tap(otherInstallation);
    final externalAdoption = find.byKey(
    ...
    final projectPackageParent = Directory(
      '${projectRoot.path}/.skillsgo/packages/github.com/skillsgo',
    );
    final projectPackageStores = projectPackageParent
        .listSync()
        .whereType<Directory>()
        .where(
          (directory) => directory.path.startsWith(
            '${projectPackageParent.path}${Platform.pathSeparator}e2e-versioned-skills@',
          ),
        )
        .toList();
    expect(projectPackageStores, hasLength(1));
    final projectPackageSkill = File(
      '${projectPackageStores.single.path}/skills/alpha/SKILL.md',
    );
    expect(projectPackageSkill.existsSync(), isTrue);
    expect(projectPackageSkill.readAsStringSync(), contains('# Alpha'));
    final managedSkillBytes = projectPackageSkill.readAsBytesSync();
    expect(
      File(
    ...
    expect(File('$sandbox/home/.agents/skills-lock.yaml').existsSync(), isTrue);
    expect(
