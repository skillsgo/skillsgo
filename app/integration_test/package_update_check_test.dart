/*
 * [INPUT]: Depends on the rendered App with bounded startup pumping, bundled CLI, JourneyRuntime filesystem/Hub/schema isolation, and immutable v1.2.0/v1.3.0 releases of the SkillsGo-owned public versioned fixture Repository.
 * [OUTPUT]: Exports and registers a reusable Journey that verifies installation of an older Repository release, discovery of its Catalog-derived update, exact-Package update persistence, and the cleared rendered update state.
...
void registerPackageUpdatePreviewJourney() {
  testWidgets(
    ...
    runPackageUpdatePreviewJourney,
    timeout: const Timeout(Duration(minutes: 4)),
  );
}

Future<void> runPackageUpdatePreviewJourney(WidgetTester tester) async {
  ...
  try {
    ...
    final installFinder = find.byKey(const ValueKey('install-location-submit'));
    ...
    await _pumpUntil(
      tester,
      find.byKey(const Key('library-attention-updates')),
      timeout: const Duration(seconds: 10),
    );
    ...
    await _pumpUntil(tester, packageCard, timeout: const Duration(seconds: 45));
