/*
 * [INPUT]: Depends on Flutter integration_test with bounded startup pumping, the real SkillsGo App entry point, JourneyRuntime isolation, onboarding preferences, a disposable Hub/schema, the bundled CLI, and the SkillsGo-owned public versioned fixture Repository.
 * [OUTPUT]: Exports and registers a reusable Journey that verifies Package search, the stable Package-wide installation action, bundled-CLI dry-run-to-apply execution, global YAML/Lock state, global Scope Package Store, and member-symlink Package Projections.
...
void registerRepositoryInstallAllJourney() {
  testWidgets(
    ...
    runRepositoryInstallAllJourney,
    timeout: const Timeout(Duration(minutes: 4)),
  );
}

Future<void> runRepositoryInstallAllJourney(WidgetTester tester) async {
  ...
  try {
    ...
    expect(File('${home.path}/.agents/skills-lock.yaml').existsSync(), isTrue);
    expect(
...
Future<void> _pumpUntil(
  ...
  final diagnostics = await runtime?.diagnosticSummary() ?? '';
  expect(
    finder,
    findsWidgets,
    reason:
