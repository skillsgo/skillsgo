 ...
 * [OUTPUT]: Provides per-Journey Home/Project/Agent/PostgreSQL-schema/Hub isolation, CLI-backed Project registration, complete Gateway/process teardown, and forwarded App/Hub diagnostics while preserving the real App-to-CLI-to-Hub boundary.
...
final class JourneyRuntime {
  ...
  Future<String> diagnosticSummary() async {
    ...
    final appDiagnostics = gateway
        .recentDiagnosticLogs()
        .map((entry) => entry.formatted)
        .toList(growable: false);
    if (appDiagnostics.isNotEmpty) {
      sections.add('App diagnostics:\n${appDiagnostics.join('\n')}');
    }
    final hubLog = File('${sandbox.path}/hub/hub.log');
    if (hubLog.existsSync()) {
      final lines = await hubLog.readAsLines();
      final start = lines.length > 120 ? lines.length - 120 : 0;
      sections.add('Hub diagnostics:\n${lines.skip(start).join('\n')}');
    }
