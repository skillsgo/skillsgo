...
Discover provides Hub-backed Search plus Cloud-backed Ranking, Trending, and Hot views. Library provides fixed Global and External Skills routes plus one location route per Added Project in its left rail. Agent selection remains a combinable multi-select filter in the content toolbar. The Library aggregates all Installation Targets under one logical Library Entry while preserving Version Divergence and exposes External Installations through their dedicated cross-location route.
...
- Library rail order is Global, External Skills, and every Added Project. Global and External Skills remain fixed at the top, only the Added Project list scrolls, and Add Project remains pinned at the bottom. External Skills uses the Folder Unknown identity and gathers External Installations from known Global and Added Project roots, while managed Global and Project routes exclude them. Fixed dividers separate the scrollable project list from both the leading destinations and footer action; neither divider moves with project scrolling. The project list uses one slim, rounded desktop scrollbar that does not compete with project labels or selected capsules; the platform must not add a second hover scrollbar. Added Project rows use a compact desktop density while the fixed destinations and Add Project retain their larger navigation targets. When no Added Project exists, the project section shows a small centered hyperlink-style Add Project entry without an icon; it invokes the same picker as the pinned action and disappears after a project is added. The toolbar owns search, update status, and a combinable Agent multi-select; it does not repeat project selection. An empty Added Project uses a concise, project-name-independent title and a Browse Skills action that returns to Discover without setup guidance.
...
- Settings rail order is General, Reminders, Agents, Managed Backups, and Advanced. Managed Backups lists original installs saved during SkillsGo adoption and provides an explicit, confirmation-gated restore action. Advanced ends with an explicit local Library refresh that rescans local inventory without mutating installations.
