...
**Local Scan Notice**:
The one-time blocking explanation shown on first Library entry when configured projects are under macOS protected folders and local project or Skill-usage access may therefore trigger a system prompt. It presents the final directory name of every configured project that will be read, followed by a compact local-only privacy statement. Library entry skips the notice when no protected-folder access is expected. Deferral is persisted and enters a restricted Library with an inline continuation action, so the modal never repeats; acknowledgment permits the existing automatic local discovery and usage workflows to begin. A permission-denied project remains visible with macOS privacy-settings and retry recovery. Explanation state does not replace system authorization or disable safe local capabilities.
_Avoid_: surprise permission prompt, privacy policy acceptance, telemetry consent
...
**Package Detail**:
The Package-centered Discover surface built from the existing CLI-mediated version-preserving Find response. It opens within the current Discover tab, reuses the full Package result card above the optional commit-pinned README loaded directly from its Hub-selected public CDN URL, and preserves the underlying search or Skill detail context for Back. Complete paginated Skill membership remains available to the aggregate installation action but is not rendered as a separate detail list. README absence or failure never blocks Package identity or installation actions, and nested Markdown stays within the same repository revision.
_Avoid_: Package Info viewer, GitHub mirror, README-backed installation contract, direct Hub HTTP
...
**Added Project**:
A local directory persisted in the CLI-owned user configuration and presented as a managed Library scope. Before the project registry has been bootstrapped, the App asks the CLI to seed up to twelve existing Workspaces observed in bounded supported-Agent registries or session metadata; afterward the registry changes only through explicit add or remove operations, including when every project has been removed. SkillsGo never crawls general filesystem roots to guess projects, and removing an Added Project only stops managing that directory.
_Avoid_: recurring automatic discovery, filesystem crawl, session transcript
