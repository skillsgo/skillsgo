/*
 * [INPUT]: Depends on native-runner-forwarded arguments including Windows Velopack fast-exit hooks, the Velopack-backed App updater and guarded production/rehearsal source-channel pairs, Dart Zones/UI dispatch, Flutter desktop bindings, the SkillsGo semantic theme, native window integration, Marionette instrumentation, App logging, SkillsGateway injection, and optional initial task presentation data.
 * [OUTPUT]: Initializes Velopack at the earliest process boundary, exits before UI initialization for official lifecycle hooks, optionally executes a loopback-only unsigned update rehearsal, otherwise starts or replaces SkillsGo through main or runSkillsGoApp with the shared production updater/feed/channel, failure/lifecycle capture, first-frame presentation, native window initialization, Hub defaults, injectable Gateway and task presentation, and debug measurements.
...
const _debugHubBaseUrl = String.fromEnvironment(
  'SKILLSGO_HUB_URL',
  defaultValue: 'https://hub.skillsgo.ai',
);
...
Future<void> main(List<String> arguments) async {
  ...
  final launch = runZonedGuarded(
    () => runSkillsGoApp(
      ...
      appUpdateChannel: appUpdateProductionChannel(),
...
Future<void> runSkillsGoApp({
  ...
  List<InstallationTask> initialInstallationTasks = const [],
