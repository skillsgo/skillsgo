...
- `skills_gateway.dart`: defines the stable application-facing Gateway interface, including one Hub Origin, batch Package-scoped Find, atomic Package installation, and adoption-backup recovery, and re-exports the complete domain vocabulary.
...
- `library_models.dart`: owns Agent catalogs, Added Projects, onboarding state, Skill detail, durable Adoption backup records, unified Library entries with optional lock-backed Adoption Package hints, and Batch Adoption scope/plan/result values.
