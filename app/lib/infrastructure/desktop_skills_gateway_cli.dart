 ...
 * [OUTPUT]: Provides coalesced non-destructive CLI compatibility detection, CLI Server lifecycle, structured command execution, analytics invalidation forwarding, safe-read transport recovery, and dead-session replacement.
...
mixin _DesktopSkillsGatewayCli on _DesktopSkillsGatewayCore {
  ...
  Future<CliServerSession> _requireCliServer(String executable) async {
    ...
    try {
      ...
      await _cliAnalyticsInvalidations?.cancel();
      final analyticsSource = session is CliServerAnalyticsEventSource
          ? session as CliServerAnalyticsEventSource
          : null;
      _cliAnalyticsInvalidations = analyticsSource
          ?.watchAnalyticsInvalidations()
          .listen((event) => _analyticsInvalidations.add(event.revision));
      await _cliAnalyticsProgress?.cancel();
      _cliAnalyticsProgress = analyticsSource?.watchAnalyticsProgress().listen(
        (event) => _analyticsProgress.add(event.progress),
      );
      return session;
    } finally {
      _cliServerStart = null;
  ...
  Future<void> _closeCliServer() async {
    ...
    _cliServerStart = null;
    await _cliAnalyticsInvalidations?.cancel();
    _cliAnalyticsInvalidations = null;
    await _cliAnalyticsProgress?.cancel();
    _cliAnalyticsProgress = null;
    await current?.close();
  }
