...
- `desktop_skills_gateway_codec.dart`: owns centralized versioned/machine-document envelope validation, strict local External source evidence and other read/planning payload decoding, minimal Package-install receipt validation, argument encoding, and bounded local Skill inspection.
...
- `desktop_skills_gateway_preferences.dart`: owns App preferences including one-time randomized theme-color and wallpaper selection, persisted update-check cache, Mandatory Onboarding state, CLI user-config project adaptation, the single Hub Origin and health check, risk policy, and App-version lookup.
...
- `desktop_skills_gateway_inventory.dart`: owns Agent inspection, local Library inventory including External source evidence, and local Skill detail.
