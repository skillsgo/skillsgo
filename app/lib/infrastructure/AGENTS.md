...
- `desktop_skills_gateway.dart`: defines the production `SkillsGateway` adapter, exact App/CLI protocol handshake, stdin-capable CLI seam, shared state including retryable Hub image-policy discovery, platform pickers, internal capability composition, the shared App-side protocol-decode failure telemetry boundary, and explicit CLI-server analytics-stream teardown for reusable runtimes.
- `app_updater.dart`: adapts the community `velopack_flutter` bridge over Velopack 1.2.0 into the App's early runtime and channel-explicit check/update/restart seam, with a loopback-only unsigned rehearsal source for CI.
...
- `io_process_runner.dart`: executes one-shot CLI probes and owns the long-lived NDJSON CLI Server session with request correlation, unsolicited analytics progress and invalidations, bounded runtime, crash fan-out, optional stdout events, process-scope isolation, and sanitized completion telemetry used by real-process E2E journeys.
...
- `desktop_skills_gateway_cli.dart`: owns coalesced non-destructive bundled CLI detection, startup handshake validation, developer override persistence, lazy CLI Server creation/replacement, analytics progress/invalidation forwarding, command execution, and one-shot transport recovery for explicitly safe reads.
- `desktop_skills_gateway_preferences.dart`: owns App preferences including one-time randomized theme-color and wallpaper selection, persisted update-check cache, macOS protected-project-path preflight, one-time local-scan acceptance/deferral state and privacy-settings recovery, Mandatory Onboarding state, CLI user-config project adaptation, the single Hub Origin and health check, risk policy, and App-version lookup.
- `desktop_skills_gateway_discovery.dart`: forwards search, ranking, Hub image-policy discovery, and user-triggered Package update checks through the long-lived CLI Server using the current Hub Origin and presentation language, owns bounded-chunk candidate Find, decodes optional Package summaries and canonical pagination, and uses exact-path `show` only for remote Skill detail.
- `desktop_skills_gateway_inventory.dart`: owns Agent inspection, local Library inventory including explicit pending and local-usage evidence availability states, a bounded Hub image-policy wait and strict External Adoption hints, and local Skill detail.
- `desktop_skills_gateway_installation.dart`: groups ordinary Installation Requests by declaration scope, invokes exact-path Package Store add directly for the user-selected Package version, accepts the CLI's minimal Package-install success receipt without reinterpreting projections, sends one reviewed stdin-JSON Adoption request, lists/restores durable adoption backups, and reports App-side protocol failures through the shared telemetry boundary.
