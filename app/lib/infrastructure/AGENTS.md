...
- `desktop_skills_gateway_preferences.dart`: owns App preferences including one-time randomized wallpaper selection, persisted update-check cache, Mandatory Onboarding state, CLI user-config project adaptation, independent Hub/Cloud origins and health checks, risk policy, and App-version lookup.
