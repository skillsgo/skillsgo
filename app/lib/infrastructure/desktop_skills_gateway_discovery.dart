 ...
 * [OUTPUT]: Provides current-language unified CLI Find enriched with local target counts and versions, Hub-configured proxy-aware Package avatar decoding, source-language server-ranked Adoption candidate confidence and versions, system-proxy-aware Hub Ranking/Trending/Hot, source-pinned Package detail plus bounded jsDelivr README loading, user-triggered Package update checks, and translation-aware Git Artifact Package Version Skill detail with immutable Package size, exact Skill targets, and Package-scope version targets through `show --path`.
...
mixin _DesktopSkillsGatewayDiscovery on _DesktopSkillsGatewayCore {
  ...
  Future<PackageDetail> loadPackageDetail(
    ...
    final coordinate = version.trim().isEmpty
        ? packagePath
        : '$packagePath@${version.trim()}';
    final first = await discover(
      DiscoveryCollection.search,
      query: coordinate,
      perPage: 100,
    );
    final summary = first.module;
    if (summary == null || summary.id != packagePath) {
      throw const SkillsException(
        'Package detail returned an invalid response.',
        kind: SkillsFailureKind.invalidResponse,
      );
    }
    final skills = [...first.skills];
    var page = first.pagination.nextPage;
    while (page != null) {
      final next = await discover(
        DiscoveryCollection.search,
        query: coordinate,
        page: page,
        perPage: 100,
      );
      if (next.module?.id != packagePath) {
        throw const SkillsException(
          'Package detail returned an inconsistent page.',
          kind: SkillsFailureKind.invalidResponse,
        );
      }
      skills.addAll(next.skills);
      page = next.pagination.nextPage;
    }
    final readmeUrl = summary.readmeUrl;
    if (readmeUrl != null &&
        (readmeUrl.scheme != 'https' ||
            readmeUrl.host != 'cdn.jsdelivr.net' ||
            readmeUrl.userInfo.isNotEmpty ||
            readmeUrl.hasQuery ||
            readmeUrl.hasFragment)) {
      throw const SkillsException(
        'Package detail returned an invalid README URL.',
        kind: SkillsFailureKind.invalidResponse,
      );
    }
  ...
  Future<String> loadPackageReadme(Uri readmeUrl) async {
    if (readmeUrl.scheme != 'https' ||
        readmeUrl.host != 'cdn.jsdelivr.net' ||
        readmeUrl.userInfo.isNotEmpty ||
        readmeUrl.hasQuery ||
        readmeUrl.hasFragment) {
      throw const SkillsException(
        'Package README URL is invalid.',
    ...
    try {
      final request = await client.getUrl(readmeUrl);
      ...
      if (response.statusCode != HttpStatus.ok) {
        throw SkillsException(
          'Package README request returned HTTP ${response.statusCode}.',
      ...
      await (() async {
        ...
        onTimeout: () {
          ...
          throw const SkillsException(
            'Package README request timed out.',
            kind: SkillsFailureKind.artifactUnavailable,
          );
        },
      );
      ...
      throw SkillsException(
        'Package README could not be loaded: $error',
        kind: SkillsFailureKind.artifactUnavailable,
      );
    } finally {
      client.close(force: true);
  ...
  Future<PackageUpdateCheckResult> checkPackageUpdate(
    ...
    try {
      ...
      if (decoded is! Map<String, dynamic> ||
          ...
          decoded['packagePath'] != packagePath ||
          decoded['version'] is! String ||
          (decoded['version'] as String).isEmpty ||
          decoded['status'] is! String) {
        throw const FormatException('Invalid Package update-check result.');
      }
      final status = switch (decoded['status']) {
        'up_to_date' => PackageUpdateCheckStatus.upToDate,
        'updating' => PackageUpdateCheckStatus.updating,
        _ => throw const FormatException('Invalid Package update status.'),
      };
    ...
    try {
      ...
      if (rawPackage != null &&
          (rawPackage is! Map<String, dynamic> ||
              ...
              (rawPackage['readmeUrl'] != null &&
                  rawPackage['readmeUrl'] is! String))) {
        throw const SkillsException(
      ...
      final package = rawPackage is Map<String, dynamic>
          ? PackageSummary(
              ...
              readmeUrl: switch (rawPackage['readmeUrl']) {
  ...
  AdoptionCandidate _decodeSkillCandidate(Object? raw) {
    ...
    return AdoptionCandidate(
      ...
      imageUrl: _resolvePackageImageUrl(packagePath, imageUrl as String?),
    );
  }
