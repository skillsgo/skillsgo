/*
 * [INPUT]: Depends on the shared gateway state, SharedPreferences, secure randomness, the host platform and home directory, CLI user-config project commands, directory pickers, project inspection, URL launching, App locale, and Hub health CLI command.
 * [OUTPUT]: Provides appearance with one-time randomized theme-color and wallpaper initialization, language, reminder, onboarding, exact macOS protected-project paths for local-scan notice preflight, checked atomic local-scan decision persistence, privacy-settings recovery, Agent-session-bootstrapped CLI-owned Added Project access, single Hub Origin configuration with image-capability invalidation, risk policy, and App-version persistence operations.
...
mixin _DesktopSkillsGatewayPreferences on _DesktopSkillsGatewayCore {
  ...
  Future<String> loadFolderTheme() async {
    ...
    final saved = preferences.getString(_folderThemeKey);
    if (saved != null) return saved;
    final color =
        '#${Random.secure().nextInt(0x1000000).toRadixString(16).padLeft(6, '0').toUpperCase()}';
    await preferences.setString(_folderThemeKey, color);
    return color;
  }
  ...
  Future<LocalScanNoticeDecision> loadLocalScanNoticeDecision() async {
    final preferences = await SharedPreferences.getInstance();
    final stored = preferences.getString(_localScanNoticeDecisionKey);
    for (final decision in LocalScanNoticeDecision.values) {
      if (stored == decision.name) return decision;
    }
    return preferences.getBool(_localScanNoticeAcknowledgedKey) == true
  ...
  Future<List<String>> loadLocalScanNoticePaths() async {
    ...
    final references = await _loadManagedProjectReferences(bootstrap: false);
    final paths = <String>{};
    for (final reference in references) {
      final projectPath = p.normalize(p.absolute(reference.path));
      if (protectedRoots.any(
        (root) => projectPath == root || p.isWithin(root, projectPath),
      )) {
  ...
  Future<void> saveLocalScanNoticeDecision(
    ...
    final saved = await (await SharedPreferences.getInstance()).setString(
      _localScanNoticeDecisionKey,
      decision.name,
    );
    if (!saved) {
      throw StateError('Local scan notice decision could not be saved.');
    }
  }
  ...
  Future<List<({String name, String path})>> _loadManagedProjectReferences({
    bool bootstrap = true,
  }) async {
    final action = bootstrap ? 'bootstrap' : 'list';
  ...
  Future<void> saveHubOrigin(String origin) async {
    ...
    _imageProxyHubOrigin = null;
    _imageProxyOrigin = null;
    _hubInfoRetryAfter = null;
  }
