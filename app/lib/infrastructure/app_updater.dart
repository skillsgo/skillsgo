 ...
 * [OUTPUT]: Provides channel-explicit typed App update checks, early Velopack runtime initialization, real check/download/apply/restart execution, and re-exports guarded update-source parsing.
...
abstract interface class AppUpdater {
  ...
  Future<AppUpdateCheck> checkForUpdate(Uri source, {String? channel});
...
final class VelopackAppUpdater implements AppUpdater {
  ...
  Future<AppUpdateCheck> checkForUpdate(Uri source, {String? channel}) async {
    ...
    final update = await velopack.getLatestUpdateInfo(channel: channel);
