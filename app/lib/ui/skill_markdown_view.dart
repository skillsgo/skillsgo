/*
 * [INPUT]: Depends on flutter_markdown_plus, optional immutable document and allowed-resource-root URLs, link handlers, the shared official Mermaid.js WebView-to-PNG bridge, Dart Markdown syntax hooks, Material 3 ThemeData, url_launcher, SkillsGo typography tokens, and shared bidirectional-content detection.
 * [OUTPUT]: Provides the single theme-aware, direction-aware, selectable, bounded-relative-resource-aware, link-safe Markdown reader whose Mermaid blocks exclusively request PNGs from the shared WebView renderer.
...
class SkillMarkdownView extends StatelessWidget {
  ...
  final Uri? resourceRootUri;
  final Future<void> Function(Uri uri)? onTapLink;
  ...
  Widget build(BuildContext context) {
    ...
    final imageDirectory = documentUri?.resolve('.').toString();
    void handleLink(String? href) {
      final parsed = href == null ? null : Uri.tryParse(href);
      final resolved = parsed == null
          ? null
          : documentUri?.resolveUri(parsed) ?? parsed;
      if (resolved == null) return;
      final handler = onTapLink;
      if (handler != null) {
        handler(resolved);
      } else {
        _openExternalLink(resolved.toString());
      }
    }

    Widget resolveImage(Uri uri, String? _, String? alt) {
      final resolved = documentUri?.resolveUri(uri) ?? uri;
      final root = resourceRootUri;
      if (root != null &&
      ...
      return _BoundedNetworkImage(uri: resolved, semanticLabel: alt);
    }
    ...
    if (!scrollable) {
      final body = Padding(
        ...
        child: MarkdownBody(
          ...
          imageBuilder: documentUri == null ? null : resolveImage,
          onTapLink: (_, href, _) => handleLink(href),
        ),
      );
      if (maxHeight == null) {
...
class _BoundedNetworkImage extends StatefulWidget {
  const _BoundedNetworkImage({required this.uri, this.semanticLabel});

  final Uri uri;
  final String? semanticLabel;

  @override
  State<_BoundedNetworkImage> createState() => _BoundedNetworkImageState();
}

class _BoundedNetworkImageState extends State<_BoundedNetworkImage> {
  late Future<Uint8List> _bytes = _load();

  @override
  void didUpdateWidget(covariant _BoundedNetworkImage oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.uri != widget.uri) _bytes = _load();
  }
    ...
    try {
      ...
      await (() async {
        ...
        onTimeout: () {
          ...
          throw TimeoutException('Markdown image request timed out.');
        },
      );
