/*
 * [INPUT]: Depends on Discover route state, gateway-backed Package detail, localized collection labels, tab animation, and shared status panels.
 * [OUTPUT]: Provides Package-detail routing with installation actions, leaderboard tabs, header reveal, route status panels, and route-local UI state.
...
extension _DiscoverPackageNavigation on _DiscoverScreenState {
  void _openPackageDetail({
    ...
    if (selectedSkill == null && routeState.scrollController.hasClients) {
      selectedSkillScrollOffset = routeState.scrollController.offset;
    }
    updateState(() {
      selectedPackage = _SelectedPackageDetail(
  ...
  Future<void> _closePackageDetail() async {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      routeState.scrollController.jumpTo(
        selectedSkillScrollOffset.clamp(
          0,
