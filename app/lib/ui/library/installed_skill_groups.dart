 ...
 * [OUTPUT]: Provides grouping data, deterministic Package grouping or global evidence-aware usage ranking with source identity, one-scope Package update cards with CLI-owned removed-Skill names, resolved Package avatars, compact Package Path identity, and compact installed Skill group cards with conditionally hidden selection controls plus an optional left-aligned External Skills management action and bounded idle guidance.
        ...
        child: Row(
          children: [
            PackageAvatar(
              ...
              imageUrl: package.skill.imageUrl,
              size: 52,
              borderRadius: 14,
            ),
...
List<_InstalledSkillGroupData> _groupInstalledSkills(
  ...
  if (usageSort != _LibraryUsageSort.none) {
    int hits(InstalledSkill skill) => usageSort == _LibraryUsageSort.hits45Days
        ? skill.hits45Days
        : skill.hits90Days;
    final ordered = List<InstalledSkill>.of(skills)
      ..sort((left, right) {
        int evidenceRank(InstalledSkill skill) => switch (skill.usageState) {
          SkillUsageState.available => 0,
          SkillUsageState.loading => 1,
          SkillUsageState.unavailable => 2,
        };
        final evidenceComparison = evidenceRank(
          left,
        ).compareTo(evidenceRank(right));
        if (evidenceComparison != 0) return evidenceComparison;
        final comparison = hits(left).compareTo(hits(right));
        if (left.usageState == SkillUsageState.available && comparison != 0) {
          return usageSortDescending ? -comparison : comparison;
        }
        final nameComparison = compareNatural(left.name, right.name);
        if (nameComparison != 0) return nameComparison;
        return compareNatural(left.packagePath, right.packagePath);
      });
    return [
  ...
  final groups = managed.entries.map((entry) {
...
class _InstalledSkillGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shimmerHighlight =
        ...
        buttonForeground;
    final header = Row(
      key: ValueKey('library-package-group-${group.source}'),
      children: [
        SizedBox(
          width: 44,
          child: Center(
            child: PackageAvatar(
              source: group.source,
              imageUrl: group.skills.firstOrNull?.imageUrl,
              size: 28,
              borderRadius: 9,
            ),
          ),
        ...
        const SizedBox(width: 8),
        _LibraryCountBadge(
          key: ValueKey('library-package-count-${group.source}'),
          ...
          children: [
            for (var index = 0; index < group.skills.length; index++) ...[
              ...
              _InstalledSkillRow(
                ...
                sourceLabel: group.ranked
                    ? _rankedSourceLabel(context, group.skills[index])
                    : null,
...
String _rankedSourceLabel(BuildContext context, InstalledSkill skill) {
  if (skill.provenance != LibraryProvenance.external) {
    return _compactPackagePath(skill.packagePath);
  }
  final hintedSource = skill.adoptionPackagePath.isNotEmpty
      ? skill.adoptionPackagePath
      : skill.packagePath;
  if (hintedSource.isEmpty) {
    return '${context.l10n.libraryLocalSkills} · ${context.l10n.localSource}';
  }
  return '${context.l10n.libraryLocalSkills} · ${_compactPackagePath(hintedSource)}';
}
