...
- `library_screen_core.dart`: owns Library lifecycle, Global/External/Project navigation, empty-project entry, icon-only local-inventory refresh, reviewed Adoption execution state, controller subscriptions, and destination composition.
...
- `library_actions.dart`: coordinates refresh, all-provenance Global and dedicated External projections, project filtering, plan-authorized adoption, direct Package update followed by inventory and App-scoped update-cache refresh, and exact removal actions.
...
- `adoption_review.dart`: keeps the External sliver group alive across normal and Adoption Review modes, pins its single morphing action group while managed rows scroll, displays local source evidence separately, runs one Hub-ranked exact-name bounded CLI-mediated batch Find, displays server confidence without client reordering, and hands the reviewed Hub candidate, version, and original targets to the console execution flow.
