 ...
 * [OUTPUT]: Provides Library loading, shared-refresh reconciliation, macOS privacy-settings launch recovery, Global/Project and Agent projection, composable management/usage filtering, All Skills / Needs Attention navigation with scroll restoration, resident-budget usage-filter toggling, attention counts and update previews, three-state usage sorting, selection, Added Project, reviewed adoption, Package update, inline-confirmed removal, and detail transitions.
...
extension _LibraryActions on _LibraryScreenState {
  void _changeContentMode(_LibraryContentMode mode) {
    updateState(() {
  ...
  void _showNeedsAttention() {
    if (contentMode == _LibraryContentMode.allSkills &&
        scrollController.hasClients) {
      allSkillsScrollOffset = scrollController.offset;
    }
    _changeContentMode(_LibraryContentMode.needsAttention);
    unawaited(checkUpdates(trigger: UpdateCheckTrigger.updatesView));
  }

  void _showAllSkills() {
    if (contentMode == _LibraryContentMode.allSkills) return;
    _changeContentMode(_LibraryContentMode.allSkills);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || !scrollController.hasClients) return;
      scrollController.jumpTo(
        allSkillsScrollOffset.clamp(
          0,
  ...
  void _openUnusedAttention() {
    _changeContentMode(_LibraryContentMode.unused45Days);
  }

  void _toggleUnusedBudgetFilter() {
    _updateLibraryQuery(() {
      contentMode = _LibraryContentMode.allSkills;
      usageFilter = usageFilter == _LibraryUsageFilter.unused45Days
          ? _LibraryUsageFilter.all
          : _LibraryUsageFilter.unused45Days;
    });
  }

  void _openOtherInstallationAttention() {
    _changeContentMode(_LibraryContentMode.otherInstallation);
  }

  void _openUpdatesAttention() {
    _changeContentMode(_LibraryContentMode.updates);
    unawaited(checkUpdates(trigger: UpdateCheckTrigger.updatesView));
  }
  ...
  Future<void> _openLocalScanPrivacySettings() async {
    if (privacySettingsOpenFailed) {
      updateState(() => privacySettingsOpenFailed = false);
    }
    try {
      final opened = await widget.gateway.openLocalScanPrivacySettings();
      if (!opened) {
        throw StateError('Local scan privacy settings could not be opened.');
      }
    } on Object {
      if (mounted) updateState(() => privacySettingsOpenFailed = true);
    }
  }
  ...
  List<InstalledSkill> get _visibleSkills {
    ...
    for (final visibleSkill in _locationAndAgentProjectedSkills) {
      if (managementFilter == _LibraryManagementFilter.managed &&
          visibleSkill.provenance == LibraryProvenance.external) {
        continue;
      }
      if (managementFilter == _LibraryManagementFilter.otherInstallation &&
          visibleSkill.provenance != LibraryProvenance.external) {
        continue;
      }
      if (usageFilter == _LibraryUsageFilter.unused45Days &&
          (visibleSkill.usageState != SkillUsageState.available ||
              visibleSkill.hits45Days != 0)) {
        continue;
      }
      if (usageFilter == _LibraryUsageFilter.unused90Days &&
          (visibleSkill.usageState != SkillUsageState.available ||
              visibleSkill.hits90Days != 0)) {
        continue;
      }
  ...
  List<InstalledSkill> get _unusedAttentionSkills =>
      _locationAndAgentProjectedSkills
          .where(
            (skill) =>
                skill.usageState == SkillUsageState.available &&
                skill.hits45Days == 0,
          )
          .toList(growable: false);

  int get _externalAttentionCount => _locationAndAgentProjectedSkills
      .where((skill) => skill.provenance == LibraryProvenance.external)
      .length;

  int get _needsAttentionCount =>
      _unusedAttentionSkills.length +
      _externalAttentionCount +
      _availableUpdatePackageCount;

  List<InstalledSkill> get _availableUpdateSkills {
    final packages = <String, InstalledSkill>{};
    for (final skill in _locationAndAgentProjectedSkills) {
      ...
      packages.putIfAbsent(skill.packagePath, () => skill);
    }
    final result = packages.values.toList()
      ..sort((a, b) => a.packagePath.compareTo(b.packagePath));
    return List.unmodifiable(result);
  }

  int get _availableUpdatePackageCount => _availableUpdateSkills.length;

  void _sortByUsage(_LibraryUsageSort requested) {
    updateState(() {
      if (usageSort != requested) {
        usageSort = requested;
        usageSortDescending = true;
      } else if (usageSortDescending) {
        usageSortDescending = false;
      } else {
        usageSort = _LibraryUsageSort.none;
        usageSortDescending = true;
      }
    });
  }
