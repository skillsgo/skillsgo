/*
 * [INPUT]: Depends on native diagram/node/style models, responsive settings, and typed sequence renderer configuration.
 * [OUTPUT]: Computes Sugiyama graph geometry and configured participant/message geometry for sequence diagrams.
 * [POS]: Serves as the shared hierarchical layout implementation and the sequence-specific layout engine.
...
class SequenceLayout extends LayoutEngine {
  ...
  Size computeLayout(
    ...
    final config = diagram.sequenceConfig;
    final participantSpacingBase =
        config?.actorMargin ?? deviceConfig?.participantSpacing ?? 150.0;
    final messageSpacing =
        config?.messageMargin ?? deviceConfig?.messageSpacing ?? 50.0;
    final fontSize = config?.messageFontSize ?? deviceConfig?.fontSize ?? 14.0;

    // Measure all nodes first
    for (final node in diagram.nodes) {
      final size = measureNode(node, style);
      node.width = config?.width ?? size.width;
      node.height = config?.height ?? size.height;
    }

    // Sequence diagrams arrange participants horizontally
    final topY = config?.diagramMarginY ?? 30.0;
    ...
    final contentWidth =
        (config?.diagramMarginX ?? style.padding) * 2 +
        totalNodesWidth +
        (participantCount > 1
            ? (participantCount - 1) * participantSpacing
            : 0);
    final totalWidth = (config?.useMaxWidth ?? true)
        ? math.max(contentWidth, availableSize.width)
        : contentWidth;

    // Position participants with calculated spacing
    var currentX = config?.diagramMarginX ?? style.padding;
    ...
    final messagesHeight =
        (config?.semanticRowCount ?? diagram.edges.length) * messageSpacing;
    final totalHeight =
        topY +
        nodeHeight +
        messageStartOffset +
        ...
        (config?.diagramMarginY ?? style.padding) +
