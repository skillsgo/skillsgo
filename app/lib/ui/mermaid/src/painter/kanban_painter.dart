/*
 * [INPUT]: Depends on Flutter Canvas, responsive configuration, complete Kanban chart configuration, the native icon-pack registry, and Mermaid semantic styles.
 * [OUTPUT]: Paints native Kanban sections, registered icons, task metadata, priorities, WIP state, and configured sizing.
 * [POS]: Serves as the chart-specific renderer for Mermaid Kanban diagrams.
...
class KanbanPainter extends CustomPainter {
  ...
  void _drawColumn(
    ...
    final columnPaint = Paint()
      ..color = Color(
        style.defaultNodeStyle.fillColor ?? KanbanChartColors.columnHeader,
      ).withValues(alpha: .35)
    ...
    // Draw column header
    _drawColumnHeader(canvas, column, topLeft, columnSize.width, headerHeight);
  ...
  void _drawColumnHeader(
    ...
    final borderPaint = Paint()
      ..color = Color(
        style.defaultNodeStyle.strokeColor ?? KanbanChartColors.columnBorder,
      )
    ...
    titlePainter.paint(
      canvas,
      Offset(
        topLeft.dx + 12 + (column.icon == null ? 0 : 24),
        topLeft.dy + height / 2 - titlePainter.height / 2,
      ),
    );
    if (column.icon case final icon?) {
      _drawRegisteredIcon(
        canvas,
        icon,
        Rect.fromCenter(
          center: Offset(topLeft.dx + 21, topLeft.dy + height / 2),
          width: 18,
          height: 18,
        ),
      );
  ...
  void _drawWipBadge(Canvas canvas, int current, int limit, Offset center) {
  ...
  double _drawCard(
    ...
    // 2. Description text
    final descPainter = _descriptionPainter(
      task,
      width - padding * 2,
      fontSize,
    );
    ...
    canvas.drawRRect(cardRect.shift(const Offset(0, 2)), shadowPaint);

    // Card background
    final cardPaint = Paint()
      ..color = Color(
        style.defaultNodeStyle.fillColor ?? KanbanChartColors.cardBackground,
      )
      ..style = PaintingStyle.fill;
    ...
    final borderPaint = Paint()
      ..color = Color(
        style.defaultNodeStyle.strokeColor ?? KanbanChartColors.cardBorder,
      )
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.0;
    ...
    canvas.drawRRect(
      RRect.fromRectAndCorners(
        Rect.fromLTWH(topLeft.dx, topLeft.dy, width, priorityBarHeight),
    ...
    // Draw description
    if (task.icon case final icon?) {
      _drawRegisteredIcon(
        canvas,
        icon,
        Rect.fromLTWH(topLeft.dx + padding, currentY, 17, 17),
      );
    }
    descPainter.paint(
      canvas,
      Offset(topLeft.dx + padding + (task.icon == null ? 0 : 22), currentY),
    );
  ...
  void _drawTicketBadge(Canvas canvas, String ticket, Offset topLeft) {
    ...
    // Draw text
    textPainter.paint(canvas, Offset(topLeft.dx + 6, topLeft.dy + 5));
  }
  ...
  TextPainter _descriptionPainter(
    KanbanTask task,
    double maxWidth,
    double fontSize,
  ) {
    ...
    return TextPainter(
      text: TextSpan(
        ...
        style: TextStyle(
          fontSize: fontSize,
          color: Color(
            style.defaultNodeStyle.textColor ?? KanbanChartColors.textColor,
          ),
      ...
      maxLines: 3,
      ellipsis: '…',
    )..layout(maxWidth: math.max(1, maxWidth - (task.icon == null ? 0 : 22)));
  }

  void _drawRegisteredIcon(Canvas canvas, String identity, Rect bounds) {
    final color = Color(
      style.defaultNodeStyle.textColor ?? KanbanChartColors.textColor,
    );
    final glyph = MermaidIconRegistry.resolve(identity);
    if (glyph != null) {
      glyph.paint(canvas, bounds, color);
      return;
    }
    final leaf = identity.split(':').last;
    final painter = TextPainter(
      text: TextSpan(
        text: leaf.isEmpty ? '?' : leaf.substring(0, 1).toUpperCase(),
        style: TextStyle(color: color, fontSize: bounds.height * .8),
      ),
      textDirection: TextDirection.ltr,
    )..layout();
    painter.paint(
  ...
  double _cardHeight(KanbanTask task, double width) {
    final isMobile = deviceConfig?.deviceType == DeviceType.mobile;
    final padding = isMobile ? 10.0 : 12.0;
    final fontSize = deviceConfig?.fontSize ?? 12.0;
    final description = _descriptionPainter(
      task,
      width - padding * 2,
      fontSize,
    );
    return padding +
        ...
        description.height +
        8 +
        (task.assigned != null || task.ticket != null ? 32 : 0) +
        padding;
  }

  /// Returns the task whose ticket badge contains [position].
  KanbanTask? ticketAt(Offset position, Size size) {
    final isMobile = deviceConfig?.deviceType == DeviceType.mobile;
    final padding = kanbanData.padding;
    final headerHeight = isMobile ? 50.0 : 60.0;
    final cardSpacing = isMobile ? 8.0 : 12.0;
    ...
    final columnY =
        padding +
        (kanbanData.title == null
            ? 0
            : isMobile
            ? 50
            : 60);
    for (final column in kanbanData.columns) {
      var cardY = columnY + headerHeight + (isMobile ? 8 : 12);
      final cardWidth = kanbanData.sectionWidth - 16;
      for (final task in column.tasks) {
        final description = _descriptionPainter(
          task,
        ...
        if (task.ticket case final ticket?) {
          ...
          final ticketX =
              columnX + 8 + cardPadding + (task.assigned == null ? 0 : 30);
          final ticketY = cardY + cardPadding + 4 + 6 + description.height + 8;
          if (Rect.fromLTWH(
            ticketX,
            ticketY,
            ticketPainter.width + 12,
            20,
          ).contains(position)) {
            return task;
          }
        }
        cardY += _cardHeight(task, cardWidth) + cardSpacing;
      }
      columnX += kanbanData.sectionWidth + (isMobile ? 12 : 16);
    }
    return null;
  }
  ...
  double _calculateColumnWidth(Size size, int columnCount) {
    return kanbanData.sectionWidth;
  }
