...
- `app_shell.dart`: lazily gates first Library entry behind a one-time project-named privacy explanation only when protected-folder access is expected, persists deferral into an inline restricted Library without repeating the modal, gates clean installs through Mandatory Onboarding, then composes the desktop shell, primary destination navigation, App-scoped appearance state, and CLI recovery banner.
...
- `discover_controller.dart`: owns immutable, race-safe Discover route and Package-summary caches, search, locale reload, loading, errors, and pagination through Riverpod.
- `discover_screen.dart` and `discover/`: expose the Discover destination while hiding leaderboard search, collection rendering, navigation recovery, desktop refresh, and Package identity behind one screen library.
...
- `installation_task_controller.dart` and `installation_task_stack.dart`: own the App-scoped presentation-only installation task state and the non-blocking bottom-right Skill task stack; durable identifiers and granular progress remain a future CLI protocol responsibility.
...
- `library_controller.dart`: owns immutable Library content, stable Entry queries, targeted post-mutation reconciliation, usage-independent initial loading, live CLI analytics progress, bounded compatibility polling while background statistics remain pending, analytics-invalidation-driven usage refresh, stale-refresh, and load-error transitions through Riverpod.
...
- `package_update_check_controller.dart`: owns Package-Path-keyed manual upstream update checking, duplicate-call coalescing, exact target-Version state, and bounded publication polling shared by Discover and remote detail.
- `package_summary_card.dart`: renders the reusable full-surface Package identity, metadata, update check, and aggregate installation card shared by Discover results and Package detail.
- `package_detail_screen.dart`: presents one immutable Package Version as the shared Package card followed by an independently loaded source-pinned README whose nested Markdown and resources remain bounded to the same CDN repository revision; loaded Skill membership supports aggregate installation without a separate rendered list.
...
This module owns rendered product behavior, navigation state, accessibility semantics, localization selection, and Burrow-inspired presentation. It consumes `SkillsGateway` domain contracts and must not implement Hub HTTP, process execution, Package persistence, or local filesystem mutation.
