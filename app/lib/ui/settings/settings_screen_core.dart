...
class _SettingsScreenState extends ConsumerState<SettingsScreen>
  ...
  void initState() {
    ...
    _scheduleAutomaticAppUpdateCheck(widget.appUpdateInitialDelay);
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      _scheduleAutomaticAppUpdateCheck(Duration.zero);
    }
  }

  void _scheduleAutomaticAppUpdateCheck(Duration delay) {
    _appUpdateTimer?.cancel();
    if (widget.appUpdateSource == null) return;
    _appUpdateTimer = Timer(delay, () async {
      await _checkAppUpdate(automatic: true);
      if (mounted) {
        _scheduleAutomaticAppUpdateCheck(widget.appUpdateCheckInterval);
      }
    });
  }
