/*
 * [INPUT]: Depends on RemoteDetailScreen state, localized copy, shared Package update-check state, audit models, installation scope widgets, and Markdown presentation.
 * [OUTPUT]: Provides remote detail toolbar, loading/error/content regions, navigable immutable Package identity, user-triggered Package update checking and new-Version notice, X-inspired inline translation state, Git Artifact identity and immutable Package size metadata, and document rendering methods.
...
extension _RemoteDetailRendering on RemoteDetailScreenState {
  ...
  Widget _detailBody() {
    ...
    final updateOperation = ref.watch(
      packageUpdateOperationProvider(widget.skill.packagePath),
    );
    ...
    return SkillDetailPageBody(
        ...
        actions: Wrap(
          ...
          children: [
            SecondaryCapsuleButton(
              key: const Key('detail-check-update'),
              label: packageUpdateOperationLabel(context, updateState),
              icon:
                  updateState.phase == PackageUpdateOperationPhase.updated ||
                      updateState.phase == PackageUpdateOperationPhase.upToDate
                  ? HugeIcons.strokeRoundedCheckmarkCircle02
                  : HugeIcons.strokeRoundedRefresh,
              onPressed: updateState.busy
                  ? null
                  : () => updateOperation.check(value.version),
            ),
      ...
      contextArea: Column(
        ...
        children: [
          if (updateState.phase == PackageUpdateOperationPhase.updated &&
              updateState.version.isNotEmpty &&
              updateState.version != value.version) ...[
            _PackageUpdateReadyBanner(
              fromVersion: value.version,
              toVersion: updateState.version,
            ),
                    ...
                    SizedBox(
                      ...
                      child: Center(
                        child: Tooltip(
                          ...
                          child: index == 0
                              ? InkWell(
                                  key: const Key('detail-open-package'),
                                  onTap: () => widget.onOpenPackage(
                                    value.packagePath,
                                    value.version,
                                  ),
                                  borderRadius: BorderRadius.circular(6),
                              ...
                              : _metadataValue(items[index].value, 16),
                        ),
                      ),
                    ),
                  ],
  ...
  Widget _metadataValue(String value, double fontSize) => Text(
    value,
    maxLines: 1,
    overflow: TextOverflow.ellipsis,
    textAlign: TextAlign.center,
...
class _PackageUpdateReadyBanner extends StatelessWidget {
  const _PackageUpdateReadyBanner({
  ...
  Widget build(BuildContext context) => Semantics(
    liveRegion: true,
    child: Container(
      key: const Key('detail-update-ready'),
      ...
      child: Row(
        children: [
          ...
          Text(
            ...
            '${context.l10n.updateVersionChange(fromVersion, toVersion)}',
            style: context.skillsTypography.bodySecondary.copyWith(
              color: Theme.of(context).colorScheme.onPrimaryContainer,
