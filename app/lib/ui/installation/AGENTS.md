...
- `installation_scope_panel.dart`: loads and selects explicit location-and-Agent targets for a direct Installation Request and renders animated installed-target detail expansion.
...
- `remote_detail_rendering.dart`: renders remote-detail content, immutable Package-detail navigation, Package update checks and new-Version notices, failures, risk, and action state without silently replacing the displayed immutable document.
