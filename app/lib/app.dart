/*
 * [INPUT]: Depends on Flutter Material and image precaching, Riverpod, SkillsGateway, the native App updater and production feed, startup Cloud and appearance state, localization delegates, the App shell, brand tokens, optional initial installation-task presentation data, and the shared Mermaid WebView renderer.
 * [OUTPUT]: Provides SkillsGoApp, the persisted-language-aware localized desktop application root with App-scoped Gateway/updater/channel, automatic-check cadence and optionally seeded installation-task presentation dependencies, selected-wallpaper startup presentation readiness, always-enabled product motion, Mermaid renderer, and eager configured-Cloud initialization.
...
import 'ui/installation_task_controller.dart';
...
class SkillsGoApp extends StatelessWidget {
  const SkillsGoApp({
    ...
    this.appUpdateInitialDelay = const Duration(seconds: 10),
    this.appUpdateCheckInterval = const Duration(hours: 6),
    this.initialInstallationTasks = const [],
  });
  ...
  final List<InstallationTask> initialInstallationTasks;
  ...
  Widget build(BuildContext context) {
    return ProviderScope(
      ...
      child: _SkillsGoMaterialApp(
        ...
        appUpdateChannel: appUpdateChannel ?? appUpdateProductionChannel(),
        appUpdateInitialDelay: appUpdateInitialDelay,
        appUpdateCheckInterval: appUpdateCheckInterval,
        initialInstallationTasks: initialInstallationTasks,
      ),
    );
  }
}
...
class _SkillsGoMaterialApp extends ConsumerWidget {
  const _SkillsGoMaterialApp({
    ...
    required this.initialInstallationTasks,
  });
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return MaterialApp(
      ...
      builder: (context, child) => InstallationTaskScope(
        initialTasks: initialInstallationTasks,
        child: MermaidWebViewRendererScope(child: child!),
      ),
